...
bool CRPIUtils::SetNativeResolution(const RESOLUTION_INFO res, EGLSurface m_nativeWindow)
{
  ...
  if(GETFLAGS_GROUP(res.dwFlags) && GETFLAGS_MODE(res.dwFlags))
  {
    ...
    if (stereo_mode == RenderStereoMode::SPLIT_HORIZONTAL || stereo_mode == RenderStereoMode::SPLIT_VERTICAL)
    {
      ...
      else if (stereo_mode == RenderStereoMode::SPLIT_VERTICAL)
        property.param1 = HDMI_3D_FORMAT_SBS_HALF;
      else if (stereo_mode == RenderStereoMode::SPLIT_HORIZONTAL)
    ...
    if (success == 0)
    {
      CLog::Log(LOGDEBUG, "EGL set HDMI mode ({},{})={} {}{}", GETFLAGS_GROUP(res.dwFlags),
    ...
    {
      CLog::Log(LOGERROR, "EGL failed to set HDMI mode ({},{})={} {}{}",
  ...
  else if(!GETFLAGS_GROUP(res.dwFlags) && GETFLAGS_MODE(res.dwFlags))
  {
    ...
    if (success == 0)
    {
      CLog::Log(LOGDEBUG, "EGL set SDTV mode ({},{})={}", GETFLAGS_GROUP(res.dwFlags),
                GETFLAGS_MODE(res.dwFlags), success);
    ...
    {
      CLog::Log(LOGERROR, "EGL failed to set SDTV mode ({},{})={}", GETFLAGS_GROUP(res.dwFlags),
                GETFLAGS_MODE(res.dwFlags), success);
    }
    m_DllBcmHost->vc_tv_unregister_callback(CallbackTvServiceCallback);
    sem_destroy(&m_tv_synced);
  ...
  if (stereo_mode == RenderStereoMode::SPLIT_VERTICAL)
  ...
  CLog::Log(LOGDEBUG, "EGL set resolution {}x{} -> {}x{} @ {:2f} fps ({},{}) flags:{:x} aspect:{:2f}",
...
static void SetResolutionString(RESOLUTION_INFO &res)
{
  ...
  res.strMode = StringUtils::Format("{}x{} ({}x{}) @ {:2f}{} - Full Screen", res.iScreenWidth, res.iScreenHeight, res.iWidth, res.iHeight, res.fRefreshRate,
...
bool CRPIUtils::ProbeResolutions(std::vector<RESOLUTION_INFO> &resolutions)
{
  ...
  if(m_initDesktopRes)
  {
    ...
    CLog::Log(LOGDEBUG, "EGL initial desktop resolution {} ({:2f})", m_desktopRes.strMode.c_str(),
  ...
  }
  {
    ...
    CLog::Log(LOGDEBUG, "EGL probe resolution {}:{:x}", m_desktopRes.strMode.c_str(),
...
void CRPIUtils::SetVisible(bool enable)
{
  ...
  CLog::Log(LOGDEBUG, "CRPIUtils::EnableDispmanxWindow({})", enable);
...
void CRPIUtils::GetSupportedModes(HDMI_RES_GROUP_T group, std::vector<RESOLUTION_INFO> &resolutions)
{
  ...
  if (supported_modes)
  {
    ...
    CLog::Log(LOGDEBUG, "EGL get supported modes ({}) = {}, prefer_group={:x}, prefer_mode={:x}", group,
  ...
  if (num_modes > 0 && prefer_group != HDMI_RES_GROUP_INVALID)
  {
    ...
    for (i=0; i < num_modes; i++, tv++)
    {
      ...
      CLog::Log(LOGDEBUG, "EGL mode {}: {} ({:2f}) {}{}:{:x}", i, res.strMode, res.fPixelRatio,
...
{
  CLog::Log(LOGDEBUG, "EGL tv_service_callback ({},{},{})", reason, param1, param2);
  switch(reason)
  {
