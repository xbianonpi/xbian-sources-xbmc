...
bool CWinSystemRpiGLESContext::InitWindowSystem()
{
  ...
  //CDVDFactoryCodec::ClearHWVideoCodecs(); **KM**
...
void CWinSystemRpiGLESContext::PresentRenderImpl(bool rendered)
{
  ...
  if (gui)
  {
    ...
    CWinSystemRpi::SetVisible(gui->GetWindowManager().HasVisibleControls() || (appPlayer && appPlayer->IsRenderingGuiLayer()));
  }
...
std::unique_ptr<CVideoSync> CWinSystemRpiGLESContext::GetVideoSync(CVideoReferenceClock* clock)
{
