...
void CVNCServer::Process()
{
  ...
  bool guiFrontToBackRendering = CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_guiFrontToBackRendering;
  ...
  while (!m_bStop && !m_vncstop)
  {
    ...
    if (!m_screen->clientHead)
    {
      ...
      {
        if (guiFrontToBackRendering)
          CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_guiFrontToBackRendering = 1;

        CLog::Log(LOGDEBUG, "CVNCServer::{} - idlewait {}", __FUNCTION__, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_guiFrontToBackRendering ? "True" : "False");

        rfbProcessEvents(m_screen, 3000000);
        ...
        if (m_screen->clientHead)
        {
          ...
          if (guiFrontToBackRendering)
            CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_guiFrontToBackRendering = 0;
    ...
    for (client_ptr = m_screen->clientHead; client_ptr && !m_bStop; client_ptr = client_ptr->next)
    {
      if (m_update > 0)
      {
        ...
        {
          CLog::Log(LOGDEBUG, "CVNCServer::{} - screenupdate suspended {}", __FUNCTION__, m_update);
        }
        rfbProcessEvents(m_screen, 8000);
        break;
      }
...
void CVNCServer::UpdateFrameBuffer()
{
  ...
  if (!winsystem)
  {
    CLog::Log(LOGDEBUG, "CVNCServer::{} no winsystem", __FUNCTION__);
    return;
  }
  ...
  if (!gui)
  {
    CLog::Log(LOGDEBUG, "CVNCServer::{} no gui", __FUNCTION__);
    return;
  }
