...
bool OMXClock::OMXSetReferenceClock(bool has_audio, bool lock /* = true */)
{
  ...
  {
    CLog::Log(LOGINFO, "OMXClock using {} as reference", refClock.eClock == OMX_TIME_RefClockVideo ? "video" : "audio");
...
bool OMXClock::OMXStep(int steps /* = 1 */, bool lock /* = true */)
{
  ...
  CLog::Log(LOGDEBUG, "OMXClock::Step ({})", steps);
  return true;
}
...
bool OMXClock::OMXReset(bool has_video, bool has_audio, bool lock /* = true */)
{
  ...
  if (m_eState == OMX_TIME_ClockStateStopped)
  {
    ...
    if(clock.nWaitMask)
    {
      ...
      CLog::Log(LOGDEBUG, "OMXClock::OMXReset audio / video : {} / {} wait mask {}->{} state : {}->{}\n",
...
bool OMXClock::OMXMediaTime(double pts, bool lock /* = true*/)
{
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "OMXClock::OMXMediaTime error setting {}", index == OMX_IndexConfigTimeCurrentAudioReference ?
  ...
  CLog::Log(LOGDEBUG, "OMXClock::OMXMediaTime set config {} = {:2f}", index == OMX_IndexConfigTimeCurrentAudioReference ?
...
bool OMXClock::OMXSetSpeed(int speed, bool lock /* = true */, bool pause_resume /* = false */)
{
  ...
  CLog::Log(LOGDEBUG, "OMXClock::OMXSetSpeed({:3f}) pause_resume:{}", (double)speed / (double)DVD_PLAYSPEED_NORMAL * (1.0 + m_speedAdjust), pause_resume);
