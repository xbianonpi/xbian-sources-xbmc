...
void CRBP::LogFirmwareVersion()
{
  ...
  CLog::Log(LOGINFO, "Raspberry PI firmware version: {}", response);
  CLog::Log(LOGINFO, "ARM mem: {}MB GPU mem: {}MB MPG2:{} WVC1:{}", m_arm_mem, m_gpu_mem,
            m_codec_mpg2_enabled, m_codec_wvc1_enabled);
  CLog::Log(LOGINFO, "cache.memorysize: {}MB", CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_cacheMemSize >> 20);
  CLog::Log(LOGINFO, "cache.memorysize: {}MB libass.cache: {}MB", CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_cacheMemSize >> 20, CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_libAssCache);
...
uint32_t CRBP::WaitVsync(uint32_t target)
{
  ...
  while (!delay.IsTimePast())
  {
    if ((signed)(m_vsync_count - target) >= 0)
      break;
    if (!m_vsync_cond.wait(vlock, std::chrono::milliseconds(delay.MillisLeft())))
      break;
  }
  if ((signed)(m_vsync_count - target) < 0)
    CLog::Log(LOGDEBUG, "CRBP::{} no  vsync {}/{} display:{:x} delay:{}", __FUNCTION__, m_vsync_count, target, m_display, delay.MillisLeft());
...
static int mbox_property(int file_desc, void *buf)
{
   ...
   if (ret_val < 0)
   {
     CLog::Log(LOGERROR, "{}: ioctl_set_msg failed:{}", __FUNCTION__, ret_val);
   }
   return ret_val;
}
...
static int mbox_open()
{
   ...
   if (file_desc < 0)
     CLog::Log(LOGERROR, "{}: Can't open device file: {} ({})", __FUNCTION__, DEVICE_FILE_NAME, file_desc);
...
AVRpiZcFrameGeometry CRBP::GetFrameGeometry(uint32_t encoding, unsigned short video_width, unsigned short video_height)
{
  ...
  case MMAL_ENCODING_YUVUV128:
  {
    ...
    geo.setStripeIsYc(1);
    if (geo.getHeightY() * stripe_w > img.pitch)
    {
...
double CRBP::AdjustHDMIClock(double adjust)
{
  ...
  CLog::Log(LOGDEBUG, "CRBP::{}({:5f}) = {:5f}", __func__, adjust, m_actual_pll_adjust);
  return m_actual_pll_adjust;
}
