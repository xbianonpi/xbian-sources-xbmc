...
{
  std::unique_lock<CCriticalSection> lock(m_vsync_lock);
  m_vsync_count++;
...
uint32_t CRBP::WaitVsync(uint32_t target)
{
  ...
  while (!delay.IsTimePast())
  {
    if ((signed)(m_vsync_count - target) >= 0)
      break;
    if (!m_vsync_cond.wait(vlock, delay.GetTimeLeft()))
      break;
  }
  if ((signed)(m_vsync_count - target) < 0)
    CLog::Log(LOGDEBUG, "CRBP::{} no  vsync {}/{} display:{:x} delay:{}", __FUNCTION__, m_vsync_count, target, m_display, delay.GetTimeLeft().count());
