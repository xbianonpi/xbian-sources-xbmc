...
bool CNetwork::ConvIPv4(const std::string &address, struct sockaddr_in *sa)
{
  if (address.empty())
    return false;
...
bool CNetwork::ConvIPv6(const std::string &address, struct sockaddr_in6 *sa)
{
  if (address.empty())
    return false;
...
bool CNetwork::IsLocalHost(const std::string& hostname)
{
  ...
  if (StringUtils::EqualsNoCase(hostname, myhostname))
    return true;
...
bool CNetwork::AddrMatch(const std::string &addr, const std::string &match_ip, const std::string &match_mask)
{
  ...
  {
    struct sockaddr_in6 sa_trymatch;
    struct sockaddr_in6 sa_iface;
    struct sockaddr_in6 sa_mask;
    if (!ConvIPv6(addr, &sa_trymatch) || !ConvIPv6(match_ip, &sa_iface)
     || !ConvIPv6(match_mask, &sa_mask))
      return false;
    ...
    unsigned int m;
    for (m = 0; m < sizeof(sa_iface.sin6_addr.s6_addr);
         (sa_iface.sin6_addr.s6_addr[m] & sa_mask.sin6_addr.s6_addr[m]) ==
         (sa_trymatch.sin6_addr.s6_addr[m] & sa_mask.sin6_addr.s6_addr[m]) ? m++ : m = -1);
