...
{
  if (CDisplaySettings::Get().ResolutionInfoSize() < 2)
    return current;
  ...
  // CHANGERESOLUTION
  if (CSettings::Get().GetBool("videoplayer.adjustresolution"))
  {
    bool i_found = false;

    // if interlaced mode
    if (m_iFlags & CONF_FLAGS_INTERLACED && CSettings::Get().GetBool("videoplayer.adjustresolutioninterlaced"))
      for (size_t i = (int)RES_DESKTOP; i < CDisplaySettings::Get().ResolutionInfoSize(); i++)
      {
        const RESOLUTION_INFO info = g_graphicsContext.GetResInfo((RESOLUTION)i);
        if (!(info.dwFlags & D3DPRESENTFLAG_INTERLACED)
        ||  CSettings::Get().GetInt("videoplayer.minimumhorizontal") > info.iScreenHeight
        || (!CSettings::Get().GetBool("videoplayer.adjustallownoncea") && !(GETFLAGS_GROUP(info.dwFlags) & HDMI_RES_GROUP_CEA))
    ...
    if (!i_found)
      for (size_t i = (int)RES_DESKTOP; i < CDisplaySettings::Get().ResolutionInfoSize(); i++)
      {
        ...
        || (!CSettings::Get().GetBool("videoplayer.adjustresolutioninterlaced") && (info.dwFlags & D3DPRESENTFLAG_INTERLACED))
        || (!CSettings::Get().GetBool("videoplayer.adjustallownoncea") && !(GETFLAGS_GROUP(info.dwFlags) & HDMI_RES_GROUP_CEA))
