...
bool CAEEncoderFFmpeg::Initialize(AEAudioFormat& format, bool allow_planar_input)
{
  ...
  if (format.m_streamInfo.m_type == CAEStreamInfo::STREAM_TYPE_AC3)
  {
    ...
    m_BitRate = CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_ac3Gain ? CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_ac3Bitrate : AC3_ENCODE_BITRATE;
    codec = avcodec_find_encoder(m_CodecID);
  }
...
#endif
  {
    ...
    else if (hasUnknownFormat)
    {
      ...
      CLog::LogF(LOGWARNING, "Unknown audio format, trying first format ({})", codec->sample_fmts[0]);
#endif
      format.m_dataFormat = AE_FMT_FLOAT;
    }
    else
