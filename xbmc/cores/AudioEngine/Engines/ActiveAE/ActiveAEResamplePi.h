...
{
public:
  const char *GetName() override { return "ActiveAEResamplePi"; }
  CActiveAEResamplePi();
  ...
  bool Init(SampleConfig dstConfig, SampleConfig srcConfig, bool upmix, bool normalize, double centerMix,
            CAEChannelInfo *remapLayout, AEQuality quality, bool force_resample, float sublevel) override;
  ...
  bool WantsNewSamples(int samples) override { return GetBufferedSamples() <= samples; }
