...
#define LOGTIME(n) \
  ...
  CLog::Log(LOGINFO, "ActiveAE::{} {} - resample {} took {:0f}ms", __FUNCTION__, n, _filename, \
            (End - Start) * 1e-6); \
  Start = End;
#else
#define LOGTIMEINIT(f)
...
bool CActiveAEResamplePi::Init(SampleConfig dstConfig, SampleConfig srcConfig, bool upmix, bool normalize, double centerMix,
                               CAEChannelInfo *remapLayout, AEQuality quality, bool force_resample, float sublevel)
{
  LOGTIMEINIT("x");

  CLog::Log(LOGINFO,
            "{}::{} remap:{:p} chan:{}->{} rate:{}->{} format:{}->{} bits:{}->{} dither:{}->{} "
            "norm:{} upmix:{}",
...
// this code is just uses ffmpeg to produce the 8x8 mixing matrix
{
  ...
  if (!m_omx_mixer.Initialize("OMX.broadcom.audio_mixer", OMX_IndexParamAudioInit))
    CLog::Log(LOGERROR, "{}::{} - m_omx_mixer.Initialize omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
  ...
  if (m_force_resample)
  {
    ...
    if (omx_err != OMX_ErrorNone)
      CLog::Log(LOGERROR, "{}::{} - error m_omx_mixer Failed to set OMX_IndexParamBrcmTimeScale omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
    m_ratio = 1.0;
  }
  ...
  if (omx_err != OMX_ErrorNone)
    CLog::Log(LOGERROR, "{}::{} - error m_omx_mixer in SetParameter omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
  ...
  if (omx_err != OMX_ErrorNone)
    CLog::Log(LOGERROR, "{}::{} - error m_omx_mixer out SetParameter omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
  ...
  if (omx_err != OMX_ErrorNone)
  {
    CLog::Log(
        LOGERROR,
        "{}::{} - error setting mixer OMX_IndexConfigBrcmAudioDownmixCoefficients, error 0x{:08x}",
...
{
  #ifdef DEBUG_VERBOSE
  CLog::Log(LOGINFO, "{}::{} samples:{}->{} (:2f})", CLASSNAME, __func__, src_samples, dst_samples, ratio);
  #endif
  if (!m_Initialized)
    return 0;
  OMX_ERRORTYPE omx_err   = OMX_ErrorNone;
  ...
  while (1)
  {
    ...
    if (omx_buffer == NULL)
    {
      CLog::Log(LOGERROR, "{}::{} m_omx_mixer.GetInputBuffer failed to get buffer", CLASSNAME, __func__);
      return false;
    }
    ...
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} OMX_EmptyThisBuffer() failed with result(0x{:x})", CLASSNAME, __func__, omx_err);
    ...
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} m_omx_mixer.FillThisBuffer result(0x{:x})", CLASSNAME, __func__, omx_err);
    ...
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} m_omx_mixer.WaitForOutputDone result(0x{:x})", CLASSNAME, __func__, omx_err);
      return false;
    }
    ...
    {
      CLog::Log(LOGERROR, "{}::{} m_omx_mixer.BadState", CLASSNAME, __func__);
      return false;
    }
    if (sent < src_samples)
      CLog::Log(LOGERROR, "{}::{} More data to send {}/{}", CLASSNAME, __func__, sent, src_samples);
  }
  #ifdef DEBUG_VERBOSE
  CLog::Log(LOGINFO, "{}::{} format:{}->{} rate:{}->{} chan:{}->{} samples {}->{} ({:f}) {}", CLASSNAME, __func__,
...
int64_t CActiveAEResamplePi::GetDelay(int64_t base)
{
  ...
  #ifdef DEBUG_VERBOSE
  CLog::Log(LOGINFO, "{}::{} = {}" PRId64, CLASSNAME, __func__, ret);
  #endif
  return ret;
}
...
int CActiveAEResamplePi::GetBufferedSamples()
{
  ...
  #ifdef DEBUG_VERBOSE
  CLog::Log(LOGINFO, "{}::{} = {}", CLASSNAME, __func__, samples);
  #endif
  return samples;
}
...
int CActiveAEResamplePi::CalcDstSampleCount(int src_samples, int dst_rate, int src_rate)
{
  ...
  #ifdef DEBUG_VERBOSE
  CLog::Log(LOGINFO, "{}::{} = {}", CLASSNAME, __func__, ret);
  #endif
  return ret;
}
