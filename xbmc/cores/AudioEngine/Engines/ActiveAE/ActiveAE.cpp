...
CSampleBuffer* CActiveAE::SyncStream(CActiveAEStream *stream)
{
  ...
  if (stream->m_resampleMode)
  {
    if (stream->m_processingBuffers)
    {
      ...
      if (stream->m_pllAdjust > 0.0f)
      {
        e = std::max(std::min(error / 50.0, 1.0), - 1.0);
        pllAdjustRequest = 1.0 + e * (double)stream->m_pllAdjust;
      }
      ...
      CLog::Log(LOGDEBUG, "ActiveAE::{} pll:{:5f} (act:{:5f} lim:{:5f}) rr:{:5f} threshold:{:3f} error:{:6f}", __FUNCTION__, pllAdjustRequest, pllAdjustActual, stream->m_pllAdjust, stream->m_processingBuffers->GetRR(), m_settings.atempoThreshold, error );
    }
  }
