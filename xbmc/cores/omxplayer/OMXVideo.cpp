...
//#include "windowing/WinSystem.h" // **KM**DeleteMe
...
bool COMXVideo::PortSettingsChanged(ResolutionUpdateInfo &resinfo)
{
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "{}::{} - error m_omx_decoder.GetParameter(OMX_IndexParamPortDefinition) omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
  }
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "{}::{} - error m_omx_decoder.GetParameter(OMX_IndexParamBrcmPixelAspectRatio) omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
  }
  ...
  CLog::Log(LOGDEBUG, "{}::{} - {}x{}@{:2f} interlace:{} deinterlace:{}", CLASSNAME, __func__,
  ...
  if(m_hdmi_clock_sync)
  {
    ...
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} - OMX_IndexConfigLatencyTarget omx_err(0{:08x})", CLASSNAME, __func__, omx_err);
      return false;
    }
  }
  ...
  if (m_deinterlace && interlace_method != VS_INTERLACEMETHOD_NONE)
  {
    ...
    if (!advanced_deinterlace)
    {
      ...
      if(omx_err != OMX_ErrorNone)
      {
        CLog::Log(LOGERROR, "{}::{} error OMX_IndexParamBrcmExtraBuffers omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
        return false;
      }
    }
    ...
    if(omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} - OMX_IndexConfigCommonImageFilterParameters omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
      return false;
    }
  }
  else
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "{}::{} - m_omx_tunnel_clock.Establish omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
    return false;
  }
  ...
  if(m_deinterlace)
  {
    ...
    if(omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} - m_omx_tunnel_image_fx.Establish omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
      return false;
    }
    ...
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} - m_omx_image_fx.SetStateForComponent omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
      return false;
    }
  }
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "{}::{} - m_omx_tunnel_sched.Establish omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
    return false;
  }
  ...
  if (omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "{}::{} - m_omx_sched.SetStateForComponent omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
    return false;
  }
  ...
  if (omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "{}::{} - m_omx_render.SetStateForComponent omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
    return false;
  }
...
bool COMXVideo::Open(CDVDStreamInfo &hints, OMXClock *clock, bool hdmi_clock_sync)
{
  ...
  CLog::Log(LOGDEBUG,
    "{}::{} - decoder_component(0x{:p}), input_port(0x{:x}), output_port(0x{:x}) hdmiclocksync {}\n",
...
int COMXVideo::Decode(uint8_t *pData, int iSize, double dts, double pts, bool &settings_changed)
{
  ...
  if (demuxer_content && demuxer_bytes > 0)
  {
    ...
    while(demuxer_bytes)
    {
      ...
      //CLog::Log(LOGINFO, "VideD: dts:{:0f} pts:{:0f} size:{})\n", dts, pts, iSize);
      ...
      if (omx_err == OMX_ErrorNone)
      {
        if(!PortSettingsChanged(resinfo))
        {
          CLog::Log(LOGERROR, "{}::{} - error PortSettingsChanged (EventParamOrConfigChanged) omx_err(0x{:08x})\n", CLASSNAME, __func__, omx_err);
        }
      }
      lock.unlock();
...
///////////////////////////////////////////////////////////////////////////////////////////
void COMXVideo::SetVideoRect(const CRect& SrcRect, const CRect& DestRect, RenderStereoMode video_mode, RenderStereoMode display_mode, bool stereo_invert)
{
  std::unique_lock<CCriticalSection> lock (m_critSection);
  if(!m_is_open)
    return;
  ...
  if (video_mode == RenderStereoMode::SPLIT_HORIZONTAL)
    configDisplay.transform = (OMX_DISPLAYTRANSFORMTYPE)(configDisplay.transform | DISPMANX_STEREOSCOPIC_TB);
  else if (video_mode == RenderStereoMode::SPLIT_VERTICAL)
