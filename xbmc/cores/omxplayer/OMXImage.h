...
public:
  ...
  static bool DecodeJpeg(const std::string& srcFile, const std::vector<uint8_t>& buf, unsigned int maxWidth, unsigned int maxHeight, unsigned int stride, void *pixels);
  ...
  static bool CreateThumb(const std::string& srcFile, const std::vector<uint8_t>& buf, unsigned int width, unsigned int height, std::string &additional_info, const std::string& destFile);
  bool SendMessage(bool (*callback)(EGLDisplay egl_display, EGLContext egl_context, void *cookie), void *cookie);
  bool DecodeJpegToTexture(const std::string& srcFile, const std::vector<uint8_t>& buf, unsigned int width, unsigned int height, void **userdata);
  ...
  static bool GetCodingType(const std::vector<uint8_t>& buf, unsigned int &width, unsigned int &height, int &orientation, std::string &error);
private:
...
public:
  ...
  // Required overrides
  void Close();
  bool ReEncode(const std::string& srcFile, const std::vector<uint8_t>& buf, unsigned int width, unsigned int height, unsigned int maxWidth, unsigned int maxHeight, int orientation, void * &pDestBuffer, unsigned int &nDestSize);
  bool Gpu() { return m_gpu; }
protected:
