...
bool COMXImage::DecodeJpeg(const std::string& srcFile, const std::vector<uint8_t>& buf, unsigned int width, unsigned int height, unsigned int stride, void *pixels)
{
  bool ret = false;
  COMXImageDec omx_image;
  if (omx_image.Decode(buf.data(), buf.size(), width, height, stride, pixels))
  {
    ...
    CLog::Log(LOGINFO, "{}: unable to decode {} {}x{}", __func__, srcFile.c_str(), width, height);
  omx_image.Close();
  return ret;
}
...
bool COMXImage::CreateThumb(const std::string& srcFile, const std::vector<uint8_t>& buf, unsigned int maxHeight, unsigned int maxWidth, std::string &additional_info, const std::string& destFile)
{
  bool okay = false;
...
bool COMXImage::DecodeJpegToTexture(const std::string& srcFile, const std::vector<uint8_t>& buf, unsigned int width, unsigned int height, void **userdata)
{
  bool ret = false;
  ...
  if (tex->egl_image && tex->texture && omx_image.Decode(buf.data(), buf.size(), width, height, tex->egl_image))
  {
    ret = true;
    *userdata = tex;
    CLog::Log(LOGDEBUG, "{}: decoded {} {}x{}", __func__, CURL::GetRedacted(srcFile).c_str(), width, height);
  }
  else
  {
    CLog::Log(LOGINFO, "{}: unable to decode to texture {} {}x{}", __func__, CURL::GetRedacted(srcFile).c_str(), width, height);
    DestroyTexture(tex);
  }
  return ret;
}
...
bool COMXImage::GetCodingType(const std::vector<uint8_t>& buf, unsigned int &width, unsigned int &height, int &orientation, std::string &error)
{
  bool valid = false;
...
bool COMXImageReEnc::ReEncode(const std::string& srcFile, const std::vector<uint8_t>& buf, unsigned int width, unsigned int height, unsigned int maxWidth, unsigned int maxHeight, int orientation, void * &pDestBuffer, unsigned int &nDestSize)
{
  if (!m_gpu)
    return false;
  ...
  const uint8_t *demuxer_content = buf.data();
  // initial dest buffer size
  nDestSize = 0;

  if(!demuxer_content || !demuxer_bytes)
  {
    CLog::Log(LOGERROR, "{}::{} {} no input buffer", CLASSNAME, __func__, srcFile.c_str());
    return false;
  }

  if(!m_omx_decoder.Initialize("OMX.broadcom.image_decode", OMX_IndexParamImageInit))
  {
    CLog::Log(LOGERROR, "{}::{} {} error m_omx_decoder.Initialize", CLASSNAME, __func__, srcFile.c_str());
    return false;
  }
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "{}::{} {} error GetParameter:OMX_IndexParamPortDefinition omx_err(0x{:08x})", CLASSNAME, __func__, srcFile.c_str(), omx_err);
    return false;
  }
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "{}::{} {} error SetParameter:OMX_IndexParamPortDefinition omx_err(0x{:08x})", CLASSNAME, __func__, srcFile.c_str(), omx_err);
    return false;
  }
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "{}::{} {} m_omx_decoder.AllocInputBuffers result(0x{:x})", CLASSNAME, __func__, srcFile.c_str(), omx_err);
    return false;
  }
  ...
  if (omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "{}::{} {} m_omx_decoder.SetStateForComponent result(0x{:x})", CLASSNAME, __func__, srcFile.c_str(), omx_err);
    return false;
  }
  ...
  while(demuxer_bytes > 0 || !port_settings_changed || !eos)
  {
    ...
    if (demuxer_bytes)
    {
       ...
       if(omx_buffer)
       {
         ...
         if (omx_err != OMX_ErrorNone)
         {
           CLog::Log(LOGERROR, "{}::{} {} OMX_EmptyThisBuffer() failed with result(0x{:x})", CLASSNAME, __func__, srcFile.c_str(), omx_err);
           m_omx_decoder.DecoderEmptyBufferDone(m_omx_decoder.GetComponent(), omx_buffer);
           return false;
         }
      }
    ...
    if(omx_err == OMX_ErrorNone)
    {
      ...
      {
        CLog::Log(LOGERROR, "{}::{} {} HandlePortSettingChange() failed", srcFile.c_str(), CLASSNAME, __func__);
        return false;
      }
    ...
    {
      CLog::Log(LOGERROR, "{}::{} {} - image not supported", CLASSNAME, __func__, srcFile.c_str());
      return false;
    }
    else if(timeout || omx_err != OMX_ErrorTimeout)
    {
      CLog::Log(LOGERROR, "{}::{} {} WaitForEvent:OMX_EventPortSettingsChanged failed ({:x})", CLASSNAME, __func__, srcFile.c_str(), omx_err);
      return false;
    }
    ...
    if (!m_encoded_buffer && port_settings_changed && demuxer_bytes == 0)
    {
      ...
      if(omx_err != OMX_ErrorNone)
      {
        CLog::Log(LOGERROR, "{}::{} {} FillThisBuffer() failed ({:x})", CLASSNAME, __func__, srcFile.c_str(), omx_err);
        m_omx_encoder.DecoderFillBufferDone(m_omx_encoder.GetComponent(), m_encoded_buffer);
        return false;
      }
    }
    if (m_encoded_buffer)
    {
      ...
      if (omx_err != OMX_ErrorNone)
      {
        CLog::Log(LOGERROR, "{}::{} {} m_omx_encoder.WaitForOutputDone result(0x{:x})", CLASSNAME, __func__, srcFile.c_str(), omx_err);
        return false;
      }
      if (!m_encoded_buffer->nFilledLen)
      {
        CLog::Log(LOGERROR, "{}::{} {} m_omx_encoder.WaitForOutputDone no data", CLASSNAME, __func__, srcFile.c_str());
        return false;
      }
  ...
  CLog::Log(LOGDEBUG, "{}::{} : {} {}x{} -> {}x{}", CLASSNAME, __func__, srcFile.c_str(), maxWidth, maxHeight, maxWidth, maxHeight);
