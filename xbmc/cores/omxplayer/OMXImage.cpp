...
COMXImageFile *COMXImage::LoadJpeg(const std::string& texturePath)
{
  ...
  {
    CLog::Log(LOGINFO, "{}: unable to load {}", __func__, CURL::GetRedacted(texturePath).c_str());
    delete file;
    file = NULL;
  }
  return file;
...
bool COMXImage::CreateThumb(const std::string& srcFile, unsigned int maxHeight, unsigned int maxWidth, bool additional_info, const std::string& destFile)
{
  bool okay = false;
  COMXImageFile file;
  ...
  int orientation = additional_info ? 1:0;
...
OMX_IMAGE_CODINGTYPE COMXImageFile::GetCodingType(unsigned int &width, unsigned int &height, int orientation)
{
  ...
  if(!m_image_size)
  {
    CLog::Log(LOGERROR, "{}::{} {} m_image_size unexpected ({})", CLASSNAME, __func__,
              GetFilename(), m_image_size); // **KM** {:u} -> {}
  ...
  if(READ16(p) == 0xFFD8)
  {
    ...
    CLog::Log(LOGERROR, "{}::{} error unsupported image format", CLASSNAME, __func__);
  ...
  if(progressive)
  {
    CLog::Log(LOGWARNING, "{}::{} progressive images not supported by decoder", CLASSNAME,
              __func__);
  ...
  if(components > 3)
  {
    CLog::Log(LOGWARNING, "{}::{} Only YUV images are supported by decoder", CLASSNAME, __func__);
    eCompressionFormat = OMX_IMAGE_CodingMax;
  }
...
bool COMXImageFile::ReadFile(const std::string& inputFile, int orientation)
{
  ...
  {
    CLog::Log(LOGDEBUG, "{}::{} {} GetCodingType=0x{:x} ({}x{})", CLASSNAME, __func__, GetFilename(),
