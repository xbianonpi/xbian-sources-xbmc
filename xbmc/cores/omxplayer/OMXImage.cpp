...
bool COMXImage::DecodeJpeg(const std::string& srcFile, const XFILE::auto_buffer &buf, unsigned int width, unsigned int height, unsigned int stride, void *pixels)
{
  ...
  if (omx_image.Decode(reinterpret_cast<const uint8_t *>(buf.get()), buf.size(), width, height, stride, pixels))
  {
    ...
    CLog::Log(LOGINFO, "%s: unable to decode %s %dx%d", __func__, srcFile.c_str(), width, height);
  omx_image.Close();
  return ret;
}
...
bool COMXImage::CreateThumb(const std::string& srcFile, const XFILE::auto_buffer &buf, unsigned int maxHeight, unsigned int maxWidth, std::string &additional_info, const std::string& destFile)
{
  ...
  if (URIUtils::HasExtension(srcFile, ".jpg|.tbn"))
  {
    if (GetCodingType(buf, width, height, orientation, error))
    {
      ...
      CLog::Log(LOGDEBUG, "%s::%s %s %dx%d %s", CLASSNAME, __func__, srcFile.c_str(), width, height, error.c_str());
  }
  return okay;
}
...
bool COMXImage::DecodeJpegToTexture(const std::string& srcFile, const XFILE::auto_buffer &buf, unsigned int width, unsigned int height, void **userdata)
{
  ...
  if (tex->egl_image && tex->texture && omx_image.Decode(reinterpret_cast<const uint8_t *>(buf.get()), buf.size(), width, height, tex->egl_image))
  {
    ret = true;
    *userdata = tex;
    CLog::Log(LOGDEBUG, "%s: decoded %s %dx%d", __func__, CURL::GetRedacted(srcFile).c_str(), width, height);
  }
  else
  {
    CLog::Log(LOGINFO, "%s: unable to decode to texture %s %dx%d", __func__, CURL::GetRedacted(srcFile).c_str(), width, height);
    DestroyTexture(tex);
  }
  return ret;
}
...
bool COMXImageReEnc::ReEncode(const std::string& srcFile, const XFILE::auto_buffer &buf, unsigned int width, unsigned int height, unsigned int maxWidth, unsigned int maxHeight, int orientation, void * &pDestBuffer, unsigned int &nDestSize)
{
  ...
  if(!demuxer_content || !demuxer_bytes)
  {
    CLog::Log(LOGERROR, "%s::%s %s no input buffer", CLASSNAME, __func__, srcFile.c_str());
    return false;
  }

  if(!m_omx_decoder.Initialize("OMX.broadcom.image_decode", OMX_IndexParamImageInit))
  {
    CLog::Log(LOGERROR, "%s::%s %s error m_omx_decoder.Initialize", CLASSNAME, __func__, srcFile.c_str());
    return false;
  }
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "%s::%s %s error GetParameter:OMX_IndexParamPortDefinition omx_err(0x%08x)", CLASSNAME, __func__, srcFile.c_str(), omx_err);
    return false;
  }
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "%s::%s %s error SetParameter:OMX_IndexParamPortDefinition omx_err(0x%08x)", CLASSNAME, __func__, srcFile.c_str(), omx_err);
    return false;
  }
  ...
  if (omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "%s::%s %s m_omx_decoder.SetStateForComponent result(0x%x)", CLASSNAME, __func__, srcFile.c_str(), omx_err);
    return false;
  }
  ...
  while(demuxer_bytes > 0 || !port_settings_changed || !eos)
  {
    ...
    if (demuxer_bytes)
    {
       ...
       if(omx_buffer)
       {
         ...
         if (omx_err != OMX_ErrorNone)
         {
           CLog::Log(LOGERROR, "%s::%s %s OMX_EmptyThisBuffer() failed with result(0x%x)", CLASSNAME, __func__, srcFile.c_str(), omx_err);
           m_omx_decoder.DecoderEmptyBufferDone(m_omx_decoder.GetComponent(), omx_buffer);
           return false;
         }
      }
    ...
    if(omx_err == OMX_ErrorNone)
    {
      ...
      {
        CLog::Log(LOGERROR, "%s::%s %s HandlePortSettingChange() failed", srcFile.c_str(), CLASSNAME, __func__);
        return false;
      }
    ...
    {
      CLog::Log(LOGERROR, "%s::%s %s WaitForEvent:OMX_EventPortSettingsChanged failed (%x)", CLASSNAME, __func__, srcFile.c_str(), omx_err);
      return false;
    }
    ...
    if (!m_encoded_buffer && port_settings_changed && demuxer_bytes == 0)
    {
      ...
      if(omx_err != OMX_ErrorNone)
      {
        CLog::Log(LOGERROR, "%s::%s %s FillThisBuffer() failed (%x)", CLASSNAME, __func__, srcFile.c_str(), omx_err);
        m_omx_encoder.DecoderFillBufferDone(m_omx_encoder.GetComponent(), m_encoded_buffer);
        return false;
      }
    }
    if (m_encoded_buffer)
    {
      ...
      if (omx_err != OMX_ErrorNone)
      {
        CLog::Log(LOGERROR, "%s::%s %s m_omx_encoder.WaitForOutputDone result(0x%x)", CLASSNAME, __func__, srcFile.c_str(), omx_err);
        return false;
      }
      if (!m_encoded_buffer->nFilledLen)
      {
        CLog::Log(LOGERROR, "%s::%s %s m_omx_encoder.WaitForOutputDone no data", CLASSNAME, __func__, srcFile.c_str());
        return false;
      }
  ...
  CLog::Log(LOGDEBUG, "%s::%s : %s %dx%d -> %dx%d", CLASSNAME, __func__, srcFile.c_str(), maxWidth, maxHeight, maxWidth, maxHeight);
