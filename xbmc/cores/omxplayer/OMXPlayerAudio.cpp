...
OMXPlayerAudio::OMXPlayerAudio(OMXClock *av_clock, CDVDMessageQueue& parent, CProcessInfo &processInfo, double messageQueueTimeSize)
...
bool OMXPlayerAudio::OpenStream(CDVDStreamInfo hints)
{
  ...
  if(m_messageQueue.IsInited())
    m_messageQueue.Put(std::make_shared<COMXMsgAudioCodecChange>(hints, codec), 0);
  else
  {
...
{
  CLog::Log(LOGINFO, "thread end: OMXPlayerAudio::OnExit()");
}
    ...
    while(!m_bStop && data_len > 0)
    {
      ...
      while(!m_bStop)
      {
        ...
        if(!bDropPacket)
        {
          ...
          if(ret != decoded_size)
          {
            CLog::Log(LOGERROR, "error ret {} decoded_size {}\n", ret, decoded_size);
          }
        }
...
void OMXPlayerAudio::Process()
{
  ...
  while(!m_bStop)
  {
    ...
    if (pMsg->IsType(CDVDMsg::DEMUXER_PACKET))
    {
      ...
      #ifdef _DEBUG
      CLog::Log(LOGINFO, "Audio: dts:{:0f} pts:{:0f} size:{} (s:{} f:{} d:{} l:{}) s:{} {}/{} late:{},{}", pPacket->dts, pPacket->pts,
      ...
      if(Decode(pPacket, bPacketDrop, m_speed > DVD_PLAYSPEED_NORMAL || m_speed < 0))
      {
        ...
        if(m_stalled && m_omxAudio.GetCacheTime() > 0.0f)
        {
    ...
    else if (pMsg->IsType(CDVDMsg::GENERAL_SYNCHRONIZE))
    {
      ...
      // **KM** if(((CDVDMsgGeneralSynchronize*)pMsg)->Wait( 100, SYNCSOURCE_AUDIO ))
      if (std::static_pointer_cast<CDVDMsgGeneralSynchronize>(pMsg)->Wait(100ms, SYNCSOURCE_AUDIO))
        CLog::Log(LOGDEBUG, "COMXPlayerAudio - CDVDMsg::GENERAL_SYNCHRONIZED");
      else
        // **KM** m_messageQueue.Put(pMsg->Acquire(), 1); /* push back as prio message, to process other prio messages */
    ...
    else if (pMsg->IsType(CDVDMsg::GENERAL_FLUSH))
    {
      ...
      CLog::Log(LOGDEBUG, "COMXPlayerAudio - CDVDMsg::GENERAL_FLUSH({})", sync);
      m_omxAudio.Flush();
      m_stalled   = true;
      m_syncState = IDVDStreamPlayer::SYNC_STARTING;
    ...
    else if (pMsg->IsType(CDVDMsg::PLAYER_SETSPEED))
    {
      ...
      if (m_speed != std::static_pointer_cast<CDVDMsgInt>(pMsg)->m_value)
      {
        ...
        CLog::Log(LOGDEBUG, "COMXPlayerAudio - CDVDMsg::PLAYER_SETSPEED {}", m_speed);
      }
    }
    else if (pMsg->IsType(CDVDMsg::GENERAL_STREAMCHANGE))
    {
      // **KM ** COMXMsgAudioCodecChange* msg(static_cast<COMXMsgAudioCodecChange*>(pMsg));
      auto msg = std::static_pointer_cast<COMXMsgAudioCodecChange>(pMsg);
...
void OMXPlayerAudio::Flush(bool sync)
{
  ...
  // **KM** m_messageQueue.Put( new CDVDMsgBool(CDVDMsg::GENERAL_FLUSH, sync), 1);
...
{
  if(m_messageQueue.IsInited())
    // **KM** m_messageQueue.Put( new CDVDMsgInt(CDVDMsg::PLAYER_SETSPEED, speed), 1 );
...
{
  std::ostringstream s;
  s << "aq:"     << std::setw(2) << std::min(99,m_messageQueue.GetLevel() + MathUtils::round_int(100.0/8.0*(double)m_omxAudio.GetCacheTime())) << "%";
