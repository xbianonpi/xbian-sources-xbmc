...
bool OMXPlayerUnsuitable(bool m_HasVideo, bool m_HasAudio, CDVDDemux* m_pDemuxer, std::shared_ptr<CDVDInputStream> m_pInputStream, CSelectionStreams &m_SelectionStreams)
{
  ...
  if (audioDevice != "PI:Analogue" &&
      ...
      audioDevice != "Default")
  {
    CLog::Log(LOGINFO, "{} OMXPlayer unsuitable due to audio sink", __func__);
    return true;
  }
  // omxplayer doesn't handle ac3 transcode
  if (settings->GetBool(CSettings::SETTING_AUDIOOUTPUT_AC3TRANSCODE))
  {
    CLog::Log(LOGINFO, "{} OMXPlayer unsuitable due to ac3transcode", __func__);
    return true;
  }
  if (m_pDemuxer)
  {
    ...
    for (const auto &it : m_SelectionStreams.Get(StreamType::VIDEO))
    {
    ...
    {
      CLog::Log(LOGINFO, "{} OMXPlayer unsuitable due to video codec ({:x}:{}/{})", __func__, codec, num_supported, num_unsupported);
      return true;
    }
  }
  if (m_pInputStream && m_pInputStream->IsStreamType(DVDSTREAM_TYPE_DVD))
  {
    CLog::Log(LOGINFO, "{} OMXPlayer unsuitable due to dvd menus", __func__);
    return true;
  }
  return false;
}
                     ...
                     CCurrentStream m_CurrentAudio, CCurrentStream m_CurrentVideo, bool m_HasVideo, bool m_HasAudio, CProcessInfo &processInfo)
{
  ...
  unsigned int now = std::chrono::steady_clock::now().time_since_epoch().count();
  if (m_OmxPlayerState.last_check_time == 0.0 || m_OmxPlayerState.last_check_time + 20 <= now)
  {
    ...
    {
      CLog::Log(LOGINFO, "{} - Reopen stream due to interlace change ({},{})", __FUNCTION__,
        m_OmxPlayerState.interlace_method, processInfo.GetVideoSettings().m_InterlaceMethod);
    ...
    #ifdef _DEBUG
    CLog::Log(LOGDEBUG, "{}::{} M:{:6f}-{:6f} (A:{:6f} V:{:6f}) PEF:{}{}{} S:{:2f} A:{:2f}f V:{:2f}/T:{:2f} (A:{}{} V:{}{}) A:{}% V:{}%", "CVideoPlayer", __FUNCTION__,
      ...
      audio_pts == DVD_NOPTS_VALUE ? 0.0:(double)audio_fifo, video_pts == DVD_NOPTS_VALUE ? 0.0:(double)video_fifo, m_OmxPlayerState.threshold,
    ...
    if(!m_Pause && (m_OmxPlayerState.bOmxSentEOFs || not_accepts_data || (audio_fifo_high && video_fifo_high) || m_playSpeed != DVD_PLAYSPEED_NORMAL))
    {
      ...
      {
        CLog::Log(LOGDEBUG, "{}::{} Resume {:2f},{:2f} (A:{}{} V:{}{}) EOF:{} FULL:{} T:{:2f}", "CVideoPlayer", __FUNCTION__, audio_fifo, video_fifo,
    ...
    else if ((m_Pause || audio_fifo_low || video_fifo_low) && m_playSpeed == DVD_PLAYSPEED_NORMAL)
    {
      if (!m_OmxPlayerState.av_clock.OMXIsPaused())
      {
        ...
        CLog::Log(LOGDEBUG, "{}::{} Pause {:2f},{:2f} (A:{}{} V:{}{}) EOF:{} FULL:{} T:{:2f}", "CVideoPlayer", __FUNCTION__, audio_fifo, video_fifo,
          audio_fifo_low, audio_fifo_high, video_fifo_low, video_fifo_high, m_OmxPlayerState.bOmxSentEOFs, not_accepts_data, m_OmxPlayerState.threshold);
