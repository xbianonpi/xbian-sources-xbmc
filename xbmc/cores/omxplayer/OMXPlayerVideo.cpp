...
OMXPlayerVideo::OMXPlayerVideo(OMXClock *av_clock,
                               ...
                               CDVDMessageQueue& parent, CRenderManager& renderManager, CProcessInfo &processInfo, double messageQueueTimeSize)
...
bool OMXPlayerVideo::OpenStream(CDVDStreamInfo hints)
{
  ...
  if(m_messageQueue.IsInited())
    // **KM** !!!! SendMessage(new COMXMsgVideoCodecChange(hints, NULL), 0);
    SendMessage(std::make_shared<COMXMsgVideoCodecChange>(hints, nullptr), 0);
  else
  {
...
void OMXPlayerVideo::Output(double pts, bool bDropPacket)
{
  if (!m_renderManager.IsConfigured()) {
    CLog::Log(LOGINFO, "{} - renderer not configured", __FUNCTION__);
    return;
  }
  ...
  if (m_processInfo.GetVideoStereoMode() != GetStereoMode())
  {
    ...
    // **KM** m_messageParent.Put(new CDVDMsg(CDVDMsg::PLAYER_AVCHANGE));
...
// **KM** inline void OMXPlayerVideo::SendMessage(CDVDMsg* pMsg, int priority)
inline void OMXPlayerVideo::SendMessage(std::shared_ptr<CDVDMsg> pMsg, int priority)
{
  m_messageQueue.Put(pMsg, priority);
  m_processInfo.SetLevelVQ(m_messageQueue.GetLevel());
}

inline void OMXPlayerVideo::SendMessageBack(std::shared_ptr<CDVDMsg> pMsg, int priority)
{
...
// **KM ** inline MsgQueueReturnCode OMXPlayerVideo::GetMessage(CDVDMsg** pMsg, unsigned int iTimeoutInMilliSeconds, int &priority)
inline MsgQueueReturnCode OMXPlayerVideo::GetMessage(std::shared_ptr<CDVDMsg>& pMsg, std::chrono::milliseconds timeout, int &priority)
{
...
{
  double frametime = (double)DVD_TIME_BASE / (double)m_fFrameRate;
  ...
  while(!m_bStop)
  {
    auto iQueueTimeOut = std::chrono::duration_cast<std::chrono::milliseconds>(
            std::chrono::duration<double, std::micro>(m_stalled / 4 ? frametime : frametime * 10));
    ...
    // **KM** CDVDMsg* pMsg;
    // **KM** MsgQueueReturnCode ret = GetMessage(&pMsg, iQueueTimeOut, iPriority);
    std::shared_ptr<CDVDMsg> pMsg;
    MsgQueueReturnCode ret = GetMessage(pMsg, iQueueTimeOut, iPriority);

    if (MSGQ_IS_ERROR(ret) || ret == MSGQ_ABORT)
    {
      CLog::Log(LOGERROR, "OMXPlayerVideo: Got MSGQ_IS_ERROR({}) Aborting", (int)ret);
      break;
    }
    ...
    if (pMsg->IsType(CDVDMsg::GENERAL_SYNCHRONIZE))
    {
      ...
      // **KM** if(((CDVDMsgGeneralSynchronize*)pMsg)->Wait(100, SYNCSOURCE_VIDEO))
      if (std::static_pointer_cast<CDVDMsgGeneralSynchronize>(pMsg)->Wait(100ms, SYNCSOURCE_VIDEO))
        CLog::Log(LOGDEBUG, "COMXPlayerVideo - CDVDMsg::GENERAL_SYNCHRONIZED");
      else
        // **KM** SendMessage(pMsg->Acquire(), 1); /* push back as prio message, to process other prio messages */
        SendMessage(pMsg, 1); /* push back as prio message, to process other prio messages */
    }
    else if (pMsg->IsType(CDVDMsg::GENERAL_RESYNC))
    {
      // **KM **double pts = static_cast<CDVDMsgDouble*>(pMsg)->m_value;
    ...
    {
      // **KM ** m_fForcedAspectRatio = *((CDVDMsgDouble*)pMsg);
    ...
    {
      // **KM ** bool sync = static_cast<CDVDMsgBool*>(pMsg)->m_value;
      bool sync = std::static_pointer_cast<CDVDMsgBool>(pMsg)->m_value;
      CLog::Log(LOGDEBUG, "COMXPlayerVideo - CDVDMsg::GENERAL_FLUSH({})", sync);
      m_stalled = true;
      m_syncState = IDVDStreamPlayer::SYNC_STARTING;
      m_nextOverlay = DVD_NOPTS_VALUE;
      m_iCurrentPts = DVD_NOPTS_VALUE;
    ...
    {
      // **KM** if (m_speed != static_cast<CDVDMsgInt*>(pMsg)->m_value)
      if (m_speed != std::static_pointer_cast<CDVDMsgInt>(pMsg)->m_value)
      {
        // **KM** m_speed = static_cast<CDVDMsgInt*>(pMsg)->m_value;
        m_speed = std::static_pointer_cast<CDVDMsgInt>(pMsg)->m_value;
        CLog::Log(LOGDEBUG, "COMXPlayerVideo - CDVDMsg::PLAYER_SETSPEED {}", m_speed);
      }
    }
    else if (pMsg->IsType(CDVDMsg::GENERAL_STREAMCHANGE))
    {
      // **KM** COMXMsgVideoCodecChange* msg(static_cast<COMXMsgVideoCodecChange*>(pMsg));
      auto msg = std::static_pointer_cast<COMXMsgVideoCodecChange>(pMsg);
    ...
    {
      // **KM ** DemuxPacket* pPacket = ((CDVDMsgDemuxerPacket*)pMsg)->GetPacket();
      // **KM **bool bPacketDrop     = ((CDVDMsgDemuxerPacket*)pMsg)->GetPacketDrop();
      ...
      #ifdef _DEBUG
      CLog::Log(LOGINFO, "Video: dts:{:0f} pts:{:0f} size:{} (s:{} f:{} d:{} l:{}) s:{} {}/{} late:{}\n", pPacket->dts, pPacket->pts,
      ...
      while (!m_bStop)
      {
        ...
        if (m_syncState == IDVDStreamPlayer::SYNC_STARTING && !bRequestDrop && settings_changed)
        {
          ...
          msg.timestamp = pts;
          // **KM** m_messageParent.Put(new CDVDMsgType<SStartMsg>(CDVDMsg::PLAYER_STARTED, msg));
...
void OMXPlayerVideo::Flush(bool sync)
{
  m_flush = true;
  FlushMessages();
  // **KM** SendMessage(new CDVDMsgBool(CDVDMsg::GENERAL_FLUSH, sync), 1);
...
bool OMXPlayerVideo::OpenDecoder()
{
  ...
  {
    CLog::Log(LOGINFO, "OMXPlayerVideo::OpenDecoder : Invalid framerate {}, using forced 25fps and just trust timestamps\n", (int)m_fFrameRate);
    m_fFrameRate = 25;
  }
  ...
  {
    CLog::Log(LOGINFO, "OMXPlayerVideo::OpenDecoder : Video codec {} width {} height {} profile {} fps {:f}\n",
...
double OMXPlayerVideo::GetOutputDelay()
{
  ...
  if( m_fFrameRate )
    time = (time * DVD_TIME_BASE) / (double)m_fFrameRate;
  else
    time = 0.0;
...
void OMXPlayerVideo::SetVideoRect(const CRect &InSrcRect, const CRect &InDestRect)
{
  ...
  RenderStereoMode video_stereo_mode = RenderStereoMode::OFF;
  std::string stereoMode = GetStereoMode();

  if (stereoMode == "left_right")
  {
    video_stereo_mode = RenderStereoMode::SPLIT_VERTICAL;
  }
  ...
  {
    video_stereo_mode = RenderStereoMode::SPLIT_HORIZONTAL;
  }
  ...
  CLog::Log(LOGDEBUG, "OMXPlayerVideo::{} {},{},{},{} -> {},{},{},{} ({},{},{},{},{})", __func__,
...
void OMXPlayerVideo::ResolutionUpdateCallBack(uint32_t width, uint32_t height, float framerate, float display_aspect)
{
  ...
  m_fFrameRate = DVD_TIME_BASE / CDVDCodecUtils::NormalizeFrameduration((double)DVD_TIME_BASE / (double)framerate);
  m_processInfo.SetVideoFps(m_fFrameRate);
  m_processInfo.SetVideoDimensions(width, height);
  m_processInfo.SetVideoDAR((float)iDisplayWidth / (float)iDisplayHeight);

  CLog::Log(LOGDEBUG,"{} - change configuration. video:{}x{}. framerate: {:4.2f}. {}x{} format: BYPASS",
