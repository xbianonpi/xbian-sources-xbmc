...
bool COMXAudio::PortSettingsChanged()
{
  ...
  if( m_omx_mixer.IsInitialized() )
  {
    ...
    {
      CLog::Log(LOGERROR, "{}::{} - error m_omx_decoder GetParameter omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
      return false;
    }
    ...
    if(omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} - error m_omx_mixer SetParameter omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
      return false;
    }

    CLog::Log(LOGDEBUG, "{}::{} - Output bps {} samplerate {} channels {} buffer size {} bytes per second {}",
    ...
    if( m_omx_render_analog.IsInitialized() )
    {
      ...
      if(omx_err != OMX_ErrorNone)
      {
        CLog::Log(LOGERROR, "{}::{} - error m_omx_render_analog SetParameter omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
        return false;
      }
    }
    ...
    if( m_omx_render_hdmi.IsInitialized() )
    {
      ...
      if(omx_err != OMX_ErrorNone)
      {
        CLog::Log(LOGERROR, "{}::{} - error m_omx_render_hdmi SetParameter omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
        return false;
      }
    }
  }
  if( m_omx_render_analog.IsInitialized() )
  {
    ...
    if(omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} - m_omx_tunnel_clock_analog.Establish omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
      return false;
    }
  ...
  if( m_omx_render_hdmi.IsInitialized() )
  {
    ...
    if(omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} - m_omx_tunnel_clock_hdmi.Establish omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
      return false;
    }
  ...
  if( m_omx_render_analog.IsInitialized() )
  {
    ...
    memcpy((char *)audioDest.sName, "local", strlen("local"));
    omx_err = m_omx_render_analog.SetConfig(OMX_IndexConfigBrcmAudioDestination, &audioDest);
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} - m_omx_render_analog.SetConfig omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
      return false;
    }
  }
  ...
  if( m_omx_render_hdmi.IsInitialized() )
  {
    ...
    OMX_CONFIG_BRCMAUDIODESTINATIONTYPE audioDest;
    OMX_INIT_STRUCTURE(audioDest);
    memcpy((char *)audioDest.sName, "hdmi", strlen("hdmi"));
    omx_err = m_omx_render_hdmi.SetConfig(OMX_IndexConfigBrcmAudioDestination, &audioDest);
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} - m_omx_render_hdmi.SetConfig omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
      return false;
    }
  }
  ...
  if( m_omx_mixer.IsInitialized() )
  {
    ...
    CLog::Log(LOGDEBUG, "{}::{} - bits:{} mode:{} channels:{} srate:{} nopassthrough", CLASSNAME, __func__,
  ...
  else
  {
    ...
    CLog::Log(LOGDEBUG, "{}::{} - bits:{} mode:{} channels:{} srate:{} passthrough", CLASSNAME, __func__,
            0, 0, 0, 0);
  }
  ...
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "{}::{} - m_omx_tunnel_decoder.Establish omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
    return false;
  }
  ...
  if( m_omx_mixer.IsInitialized() )
  {
    ...
    if(omx_err != OMX_ErrorNone) {
      CLog::Log(LOGERROR, "{}::{} - m_omx_mixer OMX_StateExecuting omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
      return false;
    }
  }
  ...
  if( m_omx_splitter.IsInitialized() )
  {
    ...
    if(omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} - m_omx_splitter OMX_StateExecuting 0x{:08x}", CLASSNAME, __func__, omx_err);
     return false;
    }
  }
  if( m_omx_render_analog.IsInitialized() )
  {
    ...
    if(omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} - m_omx_render_analog OMX_StateExecuting omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
      return false;
    }
  }
  if( m_omx_render_hdmi.IsInitialized() )
  {
    ...
    if(omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} - m_omx_render_hdmi OMX_StateExecuting omx_err(0x{:08x})", CLASSNAME, __func__, omx_err);
      return false;
    }
  }
...
{
  #ifdef DEBUG_VERBOSE
  CLog::Log(LOGINFO, "{}::{}", CLASSNAME, __func__);
  #endif
...
bool COMXAudio::Initialize(AEAudioFormat format, OMXClock *clock, CDVDStreamInfo &hints, CAEChannelInfo channelMap, bool bUsePassthrough)
{
  ...
  if (hints.extradata)
  {
    ...
    m_extradata = (uint8_t *)malloc(hints.extradata.GetSize());
    memcpy(m_extradata, hints.extradata.GetData(), hints.extradata.GetSize());
  }
  ...
  if (!m_Passthrough)
  {
    ...
    CLog::Log(LOGINFO, "{}::{} remap:{:p} chan:{}->{} norm:{} upmix:{} %llx:%llx", CLASSNAME, __func__, remapLayout, m_src_channels, m_dst_channels, normalize, upmix, m_src_chan_layout, m_dst_chan_layout);
    ...
    for (int j=0; j < 8; j++)
    {
      ...
      CLog::Log(LOGINFO, "{}::{}  {}", CLASSNAME, __func__, s);
    }
    av_freep(&input);
    av_freep(&output);
    swr_free(&m_pContext);
  ...
  CLog::Log(LOGDEBUG, "COMXAudio::Initialize Input bps {} samplerate {} channels {} buffer size {} bytes per second {}",
  ...
  CLog::Log(LOGDEBUG, "COMXAudio::Initialize device passthrough {}", m_Passthrough);
...
bool COMXAudio::ApplyVolume(void)
{
  ...
  double fVolume = m_Mute ? CApplicationVolumeHandling::VOLUME_MINIMUM : m_CurrentVolume;
  ...
  double gain = pow(10, ((double)CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->m_ac3Gain - 12.0) / 20.0);

  const double* coeff = m_downmix_matrix;
  ...
  if (m_amplification != 1.0f)
  {
    ...
    for(size_t i = 0; i < 8*8; ++i)
      mix.coeff[i] = static_cast<unsigned int>(0x10000 * (coeff[i] * gain * 0.01));
    ...
    if(omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} - error setting decoder OMX_IndexConfigBrcmAudioDownmixCoefficients, error 0x{:08x}\n",
                CLASSNAME, __func__, omx_err);
      return false;
    }
  }
  for(size_t i = 0; i < 8*8; ++i)
    mix.coeff[i] = static_cast<unsigned int>(0x10000 * (coeff[i] * gain * fVolume * (double)m_amplification * (double)m_attenuation));

  mix.nPortIndex = m_omx_mixer.GetInputPort();
  omx_err = m_omx_mixer.SetConfig(OMX_IndexConfigBrcmAudioDownmixCoefficients8x8, &mix);
  if(omx_err != OMX_ErrorNone)
  {
    CLog::Log(LOGERROR, "{}::{} - error setting mixer OMX_IndexConfigBrcmAudioDownmixCoefficients, error 0x{:08x}\n",
              CLASSNAME, __func__, omx_err);
    return false;
  }
  CLog::Log(LOGINFO, "{}::{} - Volume={:2f} (* {:2f} * {:2f})\n", CLASSNAME, __func__, fVolume, m_amplification, m_attenuation);
  return true;
}
...
unsigned int COMXAudio::AddPackets(const void* data, unsigned int len, double dts, double pts, unsigned int frame_size, bool &settings_changed)
{
  ...
  while(demuxer_samples_sent < demuxer_samples)
  {
    ...
    if ((samples < demuxer_samples || frames > 1) && m_BitsPerSample==32 && !m_Passthrough)
    {
      ...
      //CLog::Log(LOGDEBUG, "{}::{} samples:{}/{} ps:{} ops:{} fs:{} pitch:{} filled:{} frames={}", CLASSNAME, __func__, samples, demuxer_samples, plane_size, out_plane_size, frame_size, pitch, omx_buffer->nFilledLen, frames);
      for (unsigned int sample = 0; sample < samples; )
      {
        ...
        {
          //CLog::Log(LOGDEBUG, "{}::{} copy({},{},{}) (s:{} f:{} sin:{} c:{})", CLASSNAME, __func__, dst-(uint8_t *)omx_buffer->pBuffer, src-demuxer_content, out_remaining, sample, frame, sample_in_frame, channel);
    ...
    if (omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} - OMX_EmptyThisBuffer() finally failed\n", CLASSNAME, __func__);
      m_omx_decoder.DecoderEmptyBufferDone(m_omx_decoder.GetComponent(), omx_buffer);
      return 0;
    }
    //CLog::Log(LOGINFO, "AudiD: dts:{:0f} pts:{:0f} size:{}\n", dts, pts, len);
    ...
    if (omx_err == OMX_ErrorNone)
    {
      ...
      {
        CLog::Log(LOGERROR, "{}::{} - error PortSettingsChanged omx_err(0x{:08x})\n", CLASSNAME, __func__, omx_err);
      }
    }
  }
...
float COMXAudio::GetMaxLevel(double &pts)
{
  ...
  if(m_omx_decoder.IsInitialized())
  {
    ...
    if(omx_err != OMX_ErrorNone)
    {
      CLog::Log(LOGERROR, "{}::{} - error getting OMX_IndexConfigBrcmAudioMaxSample error 0x{:08x}\n",
        CLASSNAME, __func__, omx_err);
      return 0;
    }
  }
...
void COMXAudio::SubmitEOS()
{
  ...
  if(omx_buffer == NULL)
  {
    CLog::Log(LOGERROR, "{}::{} - buffer error 0x{:08x}", CLASSNAME, __func__, omx_err);
    m_failed_eos = true;
    return;
  }
...
{
  CLog::Log(LOGDEBUG, "pcm->direction      : {}\n", direction.c_str());
  CLog::Log(LOGDEBUG, "pcm->nPortIndex     : {}\n", (int)pcm->nPortIndex);
  CLog::Log(LOGDEBUG, "pcm->eNumData       : {}\n", pcm->eNumData);
  CLog::Log(LOGDEBUG, "pcm->eEndian        : {}\n", pcm->eEndian);
  CLog::Log(LOGDEBUG, "pcm->bInterleaved   : {}\n", (int)pcm->bInterleaved);
  CLog::Log(LOGDEBUG, "pcm->nBitPerSample  : {}\n", (int)pcm->nBitPerSample);
  CLog::Log(LOGDEBUG, "pcm->ePCMMode       : {}\n", pcm->ePCMMode);
  CLog::Log(LOGDEBUG, "pcm->nChannels      : {}\n", (int)pcm->nChannels);
  CLog::Log(LOGDEBUG, "pcm->nSamplingRate  : {}\n", (int)pcm->nSamplingRate);
