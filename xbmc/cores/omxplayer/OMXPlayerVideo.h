...
protected:
  ...
  // **KM** void SendMessageBack(CDVDMsg* pMsg, int priority = 0);
  void SendMessageBack(std::shared_ptr<CDVDMsg> pMsg, int priority = 0);
  // **KM** MsgQueueReturnCode GetMessage(CDVDMsg** pMsg, unsigned int iTimeoutInMilliSeconds, int &priority);
  MsgQueueReturnCode GetMessage(std::shared_ptr<CDVDMsg>& pMsg, std::chrono::milliseconds timeout, int &priority);
  std::string GetStereoMode();
private:
public:
  OMXPlayerVideo(OMXClock *av_clock, CDVDOverlayContainer* pOverlayContainer, CDVDMessageQueue& parent, CRenderManager& renderManager, CProcessInfo &processInfo, double messageQueueTimeSize);
  ~OMXPlayerVideo();
