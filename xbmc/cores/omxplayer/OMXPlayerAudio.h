...
private:
public:
  OMXPlayerAudio(OMXClock *av_clock, CDVDMessageQueue& parent, CProcessInfo &processInfo, double messageQueueTimeSize);
  ~OMXPlayerAudio();
  bool OpenStream(CDVDStreamInfo hints) override;
  void SendMessage(std::shared_ptr<CDVDMsg> pMsg, int priority = 0) override { m_messageQueue.Put(pMsg, priority); }
