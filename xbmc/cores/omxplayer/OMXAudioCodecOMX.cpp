...
bool COMXAudioCodecOMX::Open(CDVDStreamInfo &hints)
{
  ...
  if (!pCodec)
  {
    CLog::Log(LOGDEBUG,"COMXAudioCodecOMX::Open() Unable to find codec {}", hints.codec);
    return false;
  }
  ...
  if (m_pCodecContext->request_channel_layout)
    CLog::Log(LOGINFO,"COMXAudioCodecOMX::Open() Requesting channel layout of {:x}", (unsigned)m_pCodecContext->request_channel_layout);
  ...
  if (hints.extradata)
  {
    m_pCodecContext->extradata = (uint8_t*)av_mallocz(hints.extradata.GetSize() + AV_INPUT_BUFFER_PADDING_SIZE);
...
int COMXAudioCodecOMX::Decode(unsigned char* pData, int iSize, double dts, double pts)
{
  ...
  if (m_bFirstFrame)
  {
    CLog::Log(LOGDEBUG, "COMXAudioCodecOMX::Decode({:p},{}) format={}({}) chan={} samples={} size={} data={:p},{:p},{:p},{:p},{:p},{:p},{:p},{:p}",
             (void *)pData, iSize, (void *)m_pCodecContext->sample_fmt, m_desiredSampleFormat, m_pCodecContext->channels, m_pFrame1->nb_samples,
             m_pFrame1->linesize[0], (void *)m_pFrame1->data[0], (void *)m_pFrame1->data[1], (void *)m_pFrame1->data[2], (void *)m_pFrame1->data[3],
             (void *)m_pFrame1->data[4], (void *)m_pFrame1->data[5], (void *)m_pFrame1->data[6], (void *)m_pFrame1->data[7]
             );
  }
...
int COMXAudioCodecOMX::GetData(unsigned char** dst, double &dts, double &pts)
{
  ...
  {
    CLog::Log(LOGERROR, "COMXAudioCodecOMX::GetData Unexpected change of size ({}->{})", m_frameSize, outputSize);
  ...
  if(m_pCodecContext->sample_fmt != m_desiredSampleFormat)
  {
    ...
    if(!m_pConvert)
    {
      ...
      {
        CLog::Log(LOGERROR, "COMXAudioCodecOMX::Decode - Unable to initialise convert format {} to {}", m_pCodecContext->sample_fmt, m_desiredSampleFormat);
        return 0;
      }
    }
    ...
    {
      CLog::Log(LOGERROR, "COMXAudioCodecOMX::Decode - Unable to convert format {} to {}", (int)m_pCodecContext->sample_fmt, m_desiredSampleFormat);
      outputSize = 0;
    }
  }
  else
  ...
  if (m_bFirstFrame)
  {
    CLog::Log(LOGDEBUG, "COMXAudioCodecOMX::GetData size={}/{} line={}/{} buf={:p}, desired={}", inputSize, outputSize, inLineSize, outLineSize, (void *)m_pBufferOutput, desired_size);
    m_bFirstFrame = false;
  }
...
void COMXAudioCodecOMX::BuildChannelMap()
{
  ...
  {
    CLog::Log(LOGINFO, "COMXAudioCodecOMX::GetChannelMap - FFmpeg reported {} channels, but the layout contains {} ignoring", m_pCodecContext->channels, bits);
