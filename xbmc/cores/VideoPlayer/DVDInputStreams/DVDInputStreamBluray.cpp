...
void CDVDInputStreamBluray::ProcessEvent() {
  ...
  case BD_EVENT_PLAYITEM:
    ...
    if (ret)
      m_clipStartTime = std::chrono::milliseconds(clip_start / 90);
    break;
...
bool CDVDInputStreamBluray::PosTime(int ms)
{
  ...
  {
    OpenNextStream();
    SeekMVCDemux(std::chrono::milliseconds(ms) - m_clipStartTime);
  }
  return true;
}
...
bool CDVDInputStreamBluray::SeekChapter(int ch)
{
  ...
  {
    OpenNextStream();
    SeekMVCDemux(GetChapterPos(ch) - m_clipStartTime);
  }
  return true;
}
...
void CDVDInputStreamBluray::SeekMVCDemux(std::chrono::milliseconds time)
{
  if (m_bMVCPlayback && m_pMVCDemux)
    m_pMVCDemux->SeekTime(time.count());
}
