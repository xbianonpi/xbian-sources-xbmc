...
static void dump_packet(DemuxPacket *packet, bool video, bool audio)
{
  ...
  if ((!video || !CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->CanLogComponent(LOGDUMPVIDEO)) &&
      (!audio || !CServiceBroker::GetSettingsComponent()->GetAdvancedSettings()->CanLogComponent(LOGDUMPAUDIO)))
    return;
