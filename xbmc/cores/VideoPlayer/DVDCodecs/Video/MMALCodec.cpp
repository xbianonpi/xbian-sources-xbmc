...
{
  CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} {:p}", CLASSNAME, __func__, static_cast<void*>(this));
...
void CMMALVideo::PortSettingsChanged(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer)
{
  ...
  if (m_es_format->es->video.crop.width && m_es_format->es->video.crop.height)
  {
    ...
    CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} format changed: {}x{} ({}x{}) {:2f}", CLASSNAME, __func__, m_decoded_width, m_decoded_height, m_decoded_aligned_width, m_decoded_aligned_height, m_aspect_ratio);
  }
  else
    CLog::Log(LOGERROR, "{}::{} format changed: Unexpected {}x{} ({}x{})", CLASSNAME, __func__, m_es_format->es->video.crop.width, m_es_format->es->video.crop.height, m_decoded_aligned_width, m_decoded_aligned_height);

  if (!change_dec_output_format())
    CLog::Log(LOGERROR, "{}::{} - change_dec_output_format() failed", CLASSNAME, __func__);
}
...
void CMMALVideo::dec_control_port_cb(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer)
{
  ...
  if (buffer->cmd == MMAL_EVENT_ERROR)
  {
    ...
    CLog::Log(LOGERROR, "{}::{} Error (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
  }
  else if (buffer->cmd == MMAL_EVENT_FORMAT_CHANGED)
  {
    CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} format changed", CLASSNAME, __func__);
    PortSettingsChanged(port, buffer);
  }
  else
    CLog::Log(LOGERROR, "{}::{} other (cmd:{:x} data:{:x})", CLASSNAME, __func__, buffer->cmd, *(uint32_t *)buffer->data);
...
{
  CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} port:{:p} buffer {:p}, len {} cmd:{:x}", CLASSNAME, __func__,
            static_cast<void*>(port), static_cast<void*>(buffer), buffer->length, buffer->cmd);
  mmal_buffer_header_release(buffer);
  std::unique_lock<CCriticalSection> output_lock(m_output_mutex);
...
void CMMALVideo::dec_output_port_cb(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer)
{
  if (!(buffer->cmd == 0 && buffer->length > 0))
    {
      CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} port:{:p} buffer {:p}, len {} cmd:{:x} flags:{:x}", CLASSNAME,
  ...
  if (buffer->cmd == 0)
  {
    if (buffer->length > 0)
    {
      ...
      CLog::Log(LOGDEBUG, LOGVIDEO,
                "{}::{} - omvb:{:p} mmal:{:p} len:{} dts:{:3f} pts:{:3f} flags:{:x}:{:x} pool:{:p} {}x{} "
                "({}x{}) {}x{} ({}x{}) enc:{:4}",
...
bool CMMALVideo::change_dec_output_format()
{
  ...
  MMAL_PARAMETER_VIDEO_INTERLACE_TYPE_T interlace_type = {{ MMAL_PARAMETER_VIDEO_INTERLACE_TYPE, sizeof( interlace_type )}, MMAL_InterlaceProgressive, false };
  ...
  if (status == MMAL_SUCCESS)
  {
    ...
    {
      CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} Interlace mode {}->{}", CLASSNAME, __func__, m_interlace_mode, interlace_type.eMode);
      m_interlace_mode = interlace_type.eMode;
    }
  }
  else
    CLog::Log(LOGERROR, "{}::{} Failed to query interlace type on {} (status={:x} {})", CLASSNAME, __func__, m_dec_output->name, status, mmal_status_to_string(status));
  ...
  if (status != MMAL_SUCCESS)
    CLog::Log(LOGERROR, "{}::{} Failed to enable zero copy mode on {} (status={:x} {})", CLASSNAME, __func__, m_dec_output->name, status, mmal_status_to_string(status));

  status = mmal_port_format_commit(m_dec_output);
  if (status != MMAL_SUCCESS)
  {
    CLog::Log(LOGERROR, "{}::{} Failed to commit decoder output port (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
    return false;
  }
  return true;
}
...
bool CMMALVideo::SendCodecConfigData()
{
  std::unique_lock<CCriticalSection> lock(m_sharedSection);
  MMAL_STATUS_T status;
  if (!m_dec_input_pool || !m_hints.extradata)
    return true;
  // send code config data
  MMAL_BUFFER_HEADER_T *buffer = mmal_queue_timedwait(m_dec_input_pool->queue, 500);
  if (!buffer)
  {
    CLog::Log(LOGERROR, "{}::{} - mmal_queue_get failed", CLASSNAME, __func__);
    return false;
  }

  mmal_buffer_header_reset(buffer);
  buffer->cmd = 0;
  buffer->length = std::min(m_hints.extradata.GetSize(), buffer->alloc_size);
  memcpy(buffer->data, m_hints.extradata.GetData(), buffer->length);
  buffer->flags = MMAL_BUFFER_HEADER_FLAG_FRAME_END | MMAL_BUFFER_HEADER_FLAG_CONFIG;
  CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} - {:<8p} {:<6} flags:{:x}", CLASSNAME, __func__, static_cast<void*>(buffer),
            buffer->length, buffer->flags);
  status = mmal_port_send_buffer(m_dec_input, buffer);
  if (status != MMAL_SUCCESS)
  {
    CLog::Log(LOGERROR, "{}::{} Failed send buffer to decoder input port (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
    return false;
  }
  return true;
}
...
{
  std::unique_lock<CCriticalSection> lock(m_sharedSection);
  CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} usemmal:{} options:{:x} {}x{}", CLASSNAME, __func__, CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(CSettings::SETTING_VIDEOPLAYER_USEMMAL), hints.codecOptions, hints.width, hints.height);
  ...
  switch (hints.codec)
  {
    ...
    case AV_CODEC_ID_WMV3:
      ...
      break;
    default:
      CLog::Log(LOGERROR, "{}::{} : Video codec unknown: {:x}", CLASSNAME, __func__, hints.codec);
      return false;
    break;
  }
  ...
  {
    CLog::Log(LOGWARNING, "{}::{} Codec {} is not supported", CLASSNAME, __func__, m_pFormatName);
    return false;
  }
  ...
  if (!m_pool)
  {
    CLog::Log(LOGERROR, "{}::{} Failed to create pool for video output", CLASSNAME, __func__);
    return false;
  }
  std::shared_ptr<CMMALPool> pool = std::dynamic_pointer_cast<CMMALPool>(m_pool);
  ...
  if (status != MMAL_SUCCESS)
  {
    CLog::Log(LOGERROR, "{}::{} Failed to enable decoder control port {} (status={:x} {})", CLASSNAME, __func__, MMAL_COMPONENT_DEFAULT_VIDEO_DECODER, status, mmal_status_to_string(status));
    return false;
  }
  ...
  if (status != MMAL_SUCCESS)
    CLog::Log(LOGERROR, "{}::{} Failed to disable error concealment on {} (status={:x} {})", CLASSNAME, __func__, m_dec_input->name, status, mmal_status_to_string(status));
  ...
  if (status != MMAL_SUCCESS)
    CLog::Log(LOGERROR, "{}::{} Failed to enable extra buffers on {} (status={:x} {})", CLASSNAME, __func__, m_dec_input->name, status, mmal_status_to_string(status));

  status = mmal_port_parameter_set_uint32(m_dec_input, MMAL_PARAMETER_VIDEO_INTERPOLATE_TIMESTAMPS, 1);
  if (status != MMAL_SUCCESS)
    CLog::Log(LOGERROR, "{}::{} Failed to disable interpolate timestamps mode on {} (status={:x} {})", CLASSNAME, __func__, m_dec_input->name, status, mmal_status_to_string(status));
  ...
  if (status != MMAL_SUCCESS)
    CLog::Log(LOGERROR, "{}::{} Failed to configure max num callbacks on {} (status={:x} {})", CLASSNAME, __func__, m_dec_input->name, status, mmal_status_to_string(status));

  status = mmal_port_parameter_set_boolean(m_dec_input, MMAL_PARAMETER_ZERO_COPY,  MMAL_TRUE);
  if (status != MMAL_SUCCESS)
    CLog::Log(LOGERROR, "{}::{} Failed to enable zero copy mode on {} (status={:x} {})", CLASSNAME, __func__, m_dec_input->name, status, mmal_status_to_string(status));

  status = mmal_port_format_commit(m_dec_input);
  if (status != MMAL_SUCCESS)
  {
    CLog::Log(LOGERROR, "{}::{} Failed to commit format for decoder input port {} (status={:x} {})", CLASSNAME, __func__, m_dec_input->name, status, mmal_status_to_string(status));
    return false;
  }
  ...
  if (status != MMAL_SUCCESS)
  {
    CLog::Log(LOGERROR, "{}::{} Failed to enable decoder input port {} (status={:x} {})", CLASSNAME, __func__, m_dec_input->name, status, mmal_status_to_string(status));
    return false;
  }
  ...
  if (status != MMAL_SUCCESS)
  {
    CLog::Log(LOGERROR, "{}::{} Failed to commit decoder output format (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
    return false;
  }
  ...
  if (status != MMAL_SUCCESS)
  {
    CLog::Log(LOGERROR, "{}::{} Failed to enable decoder output port (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
    return false;
  }
  ...
  if (status != MMAL_SUCCESS)
  {
    CLog::Log(LOGERROR, "{}::{} Failed to enable decoder component {} (status={:x} {})", CLASSNAME, __func__, m_dec->name, status, mmal_status_to_string(status));
    return false;
  }
  ...
  if (!m_dec_input_pool)
  {
    CLog::Log(LOGERROR, "{}::{} Failed to create pool for decoder input port (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
    return false;
  }
...
bool CMMALVideo::AddData(const DemuxPacket &packet)
{
  ...
  //  CLog::Log(LOGDEBUG, "{}::{} - {:<8p} {:<6d} dts:{:3f} pts:{:3f} ready_queue({})",
  ...
  while (iSize > 0)
  {
    ...
    CLog::Log(LOGDEBUG, LOGVIDEO,
              "{}::{} - {:<8p} {:<6}/{:<6} dts:{:3f} pts:{:3f} flags:{:x} ready_queue({})", CLASSNAME,
...
{
  CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} {}->{}", CLASSNAME, __func__, m_speed, iSpeed);
...
CDVDVideoCodec::VCReturn CMMALVideo::GetPicture(VideoPicture* picture)
{
  ...
  if (send_eos && !m_got_eos)
  {
    ...
    if (buffer)
    {
      ...
      if (status != MMAL_SUCCESS)
      {
        ...
        CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} - Send EOS ({}, {}, {})", CLASSNAME, __func__, m_got_eos, m_packet_num, m_packet_num_eos);
    }
    else
    {
      CLog::Log(LOGWARNING, "{}::{} - mmal_queue_get failed", CLASSNAME, __func__);
  ...
  XbmcThreads::EndTime<> delay(500ms);
  while (ret == CDVDVideoCodec::VC_NONE && !delay.IsTimePast())
  {
    ...
    if (ret == CDVDVideoCodec::VC_NONE)
    {
      ...
      lock.unlock();
      m_output_cond.wait(output_lock, delay.GetTimeLeft());
      lock.lock();
    }
  }
  ...
  if (ret == CDVDVideoCodec::VC_PICTURE)
  {
    ...
    //CLog::Log(LOGDEBUG, "{}::{} -  {}x{} {}x{} {}x{} {}x{} {:f},{:f}", CLASSNAME, __func__, picture->iWidth, picture->iHeight, picture->iDisplayWidth, picture->iDisplayHeight, m_decoded_width, m_decoded_height, buffer->Width(), buffer->Height(), buffer->m_aspect_ratio, m_hints.aspect);

    if (buffer->m_aspect_ratio > 0.0f)
    {
    ...
    CLog::Log(LOGINFO, LOGVIDEO,
              "{}::{} dts:{:3f} pts:{:3f} flags:{:x}:{:x} MMALBuffer:{:p} mmal_buffer:{:p}", CLASSNAME,
  ...
  if (ret == CDVDVideoCodec::VC_NONE)
    CLog::Log(LOGWARNING, "{}::{} - ret({:x}) pics({}) inputs({}) slept({:2}) queued({:2f}) ({:2f}:{:2f}) full({}) flags({:x}) preroll({}) eos({} {}/{})", CLASSNAME, __func__, ret, m_output_ready.size(), mmal_queue_length(m_dec_input_pool->queue), 500-delay.GetTimeLeft().count(), queued*1e-6, m_demuxerPts*1e-6, m_decoderPts*1e-6, full, m_codecControlFlags,  m_preroll, m_got_eos, m_packet_num, m_packet_num_eos);
  else
    CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} - ret({:x}) pics({}) inputs({}) slept({:2}) queued({:2f}) ({:2f}:{:2f}) full({}) flags({:x}) preroll({}) eos({} {}/{})", CLASSNAME, __func__, ret, m_output_ready.size(), mmal_queue_length(m_dec_input_pool->queue), 500-delay.GetTimeLeft().count(), queued*1e-6, m_demuxerPts*1e-6, m_decoderPts*1e-6, full, m_codecControlFlags,  m_preroll, m_got_eos, m_packet_num, m_packet_num_eos);
...
void CMMALVideo::SetCodecControl(int flags)
{
  ...
  if (m_codecControlFlags != flags)
    CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} {:x}->{:x}", CLASSNAME, __func__, m_codecControlFlags, flags);
  m_codecControlFlags = flags;
}

std::unique_ptr<CDVDVideoCodec> CMMALVideo::Create(CProcessInfo& processInfo)
{
  if (CServiceBroker::GetSettingsComponent()->GetSettings()->GetBool(
          CSettings::SETTING_VIDEOPLAYER_USEMMAL))
    return std::make_unique<CMMALVideo>(processInfo);
