...
void CMMALYUVBuffer::GetPlanes(uint8_t*(&planes)[YuvImage::MAX_PLANES])
{
  ...
  if (VERBOSE)
    CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} {}x{} {}x{} ({}x{} {}x{})", CLASSNAME, __FUNCTION__, geo.getStrideY(), geo.getHeightY(), geo.getStrideC(), geo.getHeightC(), Width(), Height(), AlignedWidth(), AlignedHeight());
