...
CMMALBuffer::CMMALBuffer(int id) : CVideoBuffer(id)
{
  ...
  {
    CLog::Log(LOGDEBUG, "{}::{} {:p}", CLASSNAME, __func__, static_cast<void*>(this));
  }
}
 ...
 :  m_mmal_format(encoding), m_state(state), m_input(input)
{
  ...
  if (status != MMAL_SUCCESS)
    CLog::Log(LOGERROR, "{}::{} Failed to create component {}", CLASSNAME, __func__, component_name);
  ...
  if (status != MMAL_SUCCESS)
    CLog::Log(LOGERROR, "{}::{} Failed to enable zero copy mode on {} (status={:x} {})", CLASSNAME, __func__, port->name, status, mmal_status_to_string(status));

  status = mmal_port_format_commit(port);
  if (status != MMAL_SUCCESS)
    CLog::Log(LOGERROR, "{}::{} Failed to commit format for {} (status={:x} {})", CLASSNAME, __func__, port->name, status, mmal_status_to_string(status));
  ...
  if (!m_mmal_pool)
    CLog::Log(LOGERROR, "{}::{} Failed to create pool for port {}", CLASSNAME, __func__, port->name);
  else
    {
      CLog::Log(LOGDEBUG, "{}::{} Created pool {:p} of size {} x {} for port {}", CLASSNAME, __func__,
...
CMMALPool::~CMMALPool()
{
  ...
  MMAL_PORT_T *port = m_input ? m_component->input[0] : m_component->output[0];
  CLog::Log(LOGDEBUG, "{}::{} Destroying pool {:p} for port {}", CLASSNAME, __func__,
  ...
  if (port && port->is_enabled)
  {
    ...
    if (status != MMAL_SUCCESS)
       CLog::Log(LOGERROR, "{}::{} Failed to disable port {} (status={:x} {})", CLASSNAME, __func__, port->name, status, mmal_status_to_string(status));
  }
  ...
  if (m_component && m_component->is_enabled)
  {
    ...
    if (status != MMAL_SUCCESS)
      CLog::Log(LOGERROR, "{}::{} Failed to disable component {} (status={:x} {})", CLASSNAME, __func__, m_component->name, status, mmal_status_to_string(status));
  }
...
void CMMALPool::Configure(AVPixelFormat format, int width, int height, int alignedWidth, int alignedHeight, int size)
{
  ...
  CLog::Log(LOGDEBUG, "{}::{} pool:{:p} {}x{} ({}x{}) pix:{} size:{} fmt:{:4}", CLASSNAME, __func__,
...
CMMALBuffer *CMMALPool::GetBuffer(uint32_t timeout)
{
  ...
  if (timeout > 0 && !omvb)
    {
      CLog::Log(LOGERROR, "{}::{} - failed pool:{:p} omvb:{:p} mmal:{:p} timeout:{}", CLASSNAME,
    ...
    {
      CLog::Log(LOGDEBUG,
                "{}::{} pool:{:p} omvb:{:p} mmal:{:p} gmem:{:p} new:{} id:{} to:{} {}x{} ({}x{}) size:{} "
                "pool:{:p}:{:p} enc:{:4}",
...
void CMMALPool::Prime()
{
  ...
  while (omvb = GetBuffer(0), omvb)
  {
    ...
    {
      CLog::Log(
          LOGDEBUG, "{}::{} Send omvb:{:p} mmal:{:p} from pool {:p} to {} len:{} cmd:{:x} flags:{:x} pool:{:p}",
    ...
    {
      CLog::Log(
          LOGERROR, "{}::{} - Failed to send omvb:{:p} mmal:{:p} from pool {:p} to {} (status=0{:x} {})",
          CLASSNAME, __func__, static_cast<void*>(omvb), static_cast<void*>(omvb->mmal_buffer),
...
{
  std::unique_lock<CCriticalSection> lock(m_sharedSection);
  CRenderInfo info;

  CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} opaque:{:p}", CLASSNAME, __func__, static_cast<void*>(this));
...
void CMMALRenderer::vout_input_port_cb(MMAL_PORT_T *port, MMAL_BUFFER_HEADER_T *buffer)
{
  if (VERBOSE && CServiceBroker::GetLogging().CanLogComponent(LOGVIDEO))
  {
    CLog::Log(LOGDEBUG, "{}::{} omvb:{:p} mmal:{:p} dts:{:3f} pts:{:3f} len:{} cmd:{:x} flags:{:x}",
...
bool CMMALRenderer::CheckConfigurationVout(uint32_t width, uint32_t height, uint32_t aligned_width, uint32_t aligned_height, uint32_t encoding)
{
  ...
  if (!m_vout)
  {
    ...
    CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} CreateRenderer", CLASSNAME, __func__);

    status = mmal_component_create(MMAL_COMPONENT_DEFAULT_VIDEO_RENDERER, &m_vout);
    if(status != MMAL_SUCCESS)
    {
      CLog::Log(LOGERROR, "{}::{} Failed to create vout component (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
      return false;
    }
    ...
    if (status != MMAL_SUCCESS)
      CLog::Log(LOGERROR, "{}::{} Failed to enable no image padding mode on {} (status={:x} {})", CLASSNAME, __func__, m_vout_input->name, status, mmal_status_to_string(status));
  ...
  if (m_vout_input && (sizeChanged || encodingChanged))
  {
    ...
    CLog::Log(LOGDEBUG, "{}::{} Changing Vout dimensions from {}x{} ({}x{}) to {}x{} ({}x{}) {:4}", CLASSNAME, __func__,
    ...
    if (encodingChanged && m_vout_input->is_enabled)
    {
      ...
      if (status != MMAL_SUCCESS)
      {
        CLog::Log(LOGERROR, "{}::{} Failed to disable vout input port (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
        return false;
      }
    }
    ...
    if (status != MMAL_SUCCESS)
    {
      CLog::Log(LOGERROR, "{}::{} Failed to commit vout input format (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
      return false;
    }
    ...
    if (!m_vout_input->is_enabled)
    {
      ...
      if (status != MMAL_SUCCESS)
      {
        CLog::Log(LOGERROR, "{}::{} Failed to enable vout input port (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
        return false;
      }
    }
  }
  ...
  if (m_vout && !m_vout->is_enabled)
  {
    ...
    if(status != MMAL_SUCCESS)
    {
      CLog::Log(LOGERROR, "{}::{} Failed to enable vout component (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
      return false;
    }
...
void CMMALRenderer::Process()
{
  ...
  CLog::Log(LOGDEBUG, "{}::{} - starting", CLASSNAME, __func__);
  while (!bStop)
  {
    ...
    m_error += inc;
    CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} - debug vsync:{} queue:{} fps:{:2f}/{:2f}/{:2f} inc:{:f} diff:{:f}", CLASSNAME, __func__, g_RBP.LastVsync(), mmal_queue_length(m_queue_render), fps, m_fps, dfps, inc, m_error);
    ...
    while (mmal_queue_length(m_queue_render) > 2 || (mmal_queue_length(m_queue_render) > 1 && m_error > 1.0))
    {
      ...
      else if (buffer)
      {
        ...
        omvb->Release();
        CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} - discard omvb:{:p} mmal:{:p} vsync:{} queue:{} diff:{:f}",
    ...
    if (m_error > 0.0)
    {
      ...
      else if (buffer)
      {
        ...
        if (status != MMAL_SUCCESS)
        {
          CLog::Log(LOGERROR, "{}::{} - Failed to send omvb:{:p} mmal:{:p} to {} (status=0{:x} {})",
                    CLASSNAME, __func__, static_cast<void*>(omvb), static_cast<void*>(buffer),
                    m_vout_input->name, status, mmal_status_to_string(status));
        }
      }
      CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} - omvb:{:p} mmal:{:p} vsync:{} queue:{} diff:{:f}", CLASSNAME,
  ...
  CLog::Log(LOGDEBUG, "{}::{} - stopping", CLASSNAME, __func__);
}
...
void CMMALRenderer::Run()
{
  ...
  while (1)
  {
    ...
    {
      CLog::Log(LOGDEBUG,
                "{}::{} {} omvb:{:p} mmal:{:p} dts:{:3f} pts:{:3f} len:{} cmd:{:x} flags:{:x} enc:{:4}",
    ...
    case MMALStateDeint:
    {
      if (buffer->length > 0)
      {
        if (m_queue_render)
        {
          ...
          if (VERBOSE && CServiceBroker::GetLogging().CanLogComponent(LOGVIDEO))
          {
            CLog::Log(LOGDEBUG, "{}::{} send {:p} to m_queue_render", CLASSNAME, __func__, static_cast<void*>(omvb));
          }
          kept = true;
        }
        else
        {
          ...
          if (m_vout_input)
          {
            ...
            if (VERBOSE && CServiceBroker::GetLogging().CanLogComponent(LOGVIDEO))
            {
              CLog::Log(LOGDEBUG, "{}::{} send {:p} to m_vout_input", CLASSNAME, __func__, static_cast<void*>(omvb));
            }
            MMAL_STATUS_T status = mmal_port_send_buffer(m_vout_input, buffer);
            if (status != MMAL_SUCCESS)
            {
    ...
    if (!kept)
    {
      if (VERBOSE && CServiceBroker::GetLogging().CanLogComponent(LOGVIDEO))
      {
        CLog::Log(
            LOGDEBUG,
            "{}::{} {} Not kept: omvb:{:p} mmal:{:p} dts:{:3f} pts:{:3f} len:{} cmd:{:x} flags:{:x} enc:{:4}",
            CLASSNAME, __func__, omvb ? omvb->GetStateName() : "", static_cast<void*>(omvb),
            static_cast<void*>(buffer), buffer->dts * 1e-6, buffer->pts * 1e-6, buffer->length,
            buffer->cmd, buffer->flags, omvb ? (char*)&omvb->Encoding() : "");
      }
...
void CMMALRenderer::UpdateFramerateStats(double pts)
{
  ...
  if (VERBOSE && CServiceBroker::GetLogging().CanLogComponent(LOGVIDEO))
  {
    CLog::Log(LOGDEBUG, "{}::{} pts:{:3f} diff:{:3f} m_frameInterval:{:6f} m_frameIntervalDiff:{:6f}", CLASSNAME, __func__, pts*1e-6, diff * 1e-6 , m_frameInterval * 1e-6, m_frameIntervalDiff *1e-6);
  }
}
...
void CMMALRenderer::AddVideoPicture(const VideoPicture& pic, int id)
{
  ...
  if (VERBOSE && CServiceBroker::GetLogging().CanLogComponent(LOGVIDEO))
  {
    CLog::Log(LOGDEBUG, "{}::{} MMAL - {:p} ({:p}) {}", CLASSNAME, __func__, static_cast<void*>(buffer),
...
bool CMMALRenderer::Configure(const VideoPicture &picture, float fps, unsigned int orientation)
{
  ...
  CLog::Log(LOGDEBUG, "{}::{} - {}x{}->{}x{}@{:2f} flags:{:x} format:{} orient:{}", CLASSNAME, __func__, picture.iWidth, picture.iHeight, picture.iDisplayWidth, picture.iDisplayHeight, fps, m_iFlags, m_format, orientation);
...
void CMMALRenderer::ReleaseBuffer(int id)
{
  ...
  if (VERBOSE && CServiceBroker::GetLogging().CanLogComponent(LOGVIDEO))
  {
    CLog::Log(LOGDEBUG, "{}::{} - MMAL: source:{} omvb:{:p} mmal:{:p}", CLASSNAME, __func__, id,
...
{
  CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{}", CLASSNAME, __func__);
...
void CMMALRenderer::RenderUpdate(int index, int index2, bool clear, unsigned int flags, unsigned int alpha)
{
  ...
  if (!m_bConfigured)
  {
    CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} - not configured: clear:{} flags:{:x} alpha:{} source:{}", CLASSNAME, __func__, clear, flags, alpha, index);
    goto exit;
  }

  omvb = m_buffers[index];

  if (CServiceBroker::GetWinSystem()->GetGfxContext().GetStereoView() != RenderStereoView::RIGHT)
  {
    ManageRenderArea();
    CRect view;
  ...
  if (omvb && omvb->m_rendered)
  {
    CLog::Log(
        LOGDEBUG, LOGVIDEO,
        "{}::{} - MMAL: clear:{} flags:{:x} alpha:{} source:{} omvb:{:p} mmal:{:p} mflags:{:x} skipping",
  ...
  if (omvb && omvb->m_state == MMALStateBypass)
  {
    ...
    if (VERBOSE && CServiceBroker::GetLogging().CanLogComponent(LOGVIDEO))
    {
      CLog::Log(LOGDEBUG, "{}::{} - OMX: clear:{} flags:{:x} alpha:{} source:{} omvb:{:p}", CLASSNAME,
  ...
  else if (omvb && omvb->mmal_buffer)
  {
    ...
    CLog::Log(LOGDEBUG, LOGVIDEO,
              "{}::{} - MMAL: clear:{} flags:{:x} alpha:{} source:{} omvb:{:p} mmal:{:p} mflags:{:x} "
              "len:{} data:{:p} enc:{:4}",
              CLASSNAME, __func__, clear, flags, alpha, index, static_cast<void*>(omvb),
              static_cast<void*>(omvb->mmal_buffer), omvb->mmal_buffer->flags,
  ...
  else
  {
    CLog::Log(
        LOGDEBUG,
        "{}::{} - MMAL: No buffer to update clear:{} flags:{:x} alpha:{} source:{} omvb:{:p} mmal:{:p}",
        CLASSNAME, __func__, clear, flags, alpha, index, static_cast<void*>(omvb),
...
exit:
   ...
   {
     CLog::Log(LOGDEBUG, "{}::{} - vsync {} (+{})", CLASSNAME, __func__, m_vsync_count, v - m_vsync_count);
     m_vsync_count = v + 1;
   }
   else
     m_vsync_count++;
...
void CMMALRenderer::UnInitMMAL()
{
  ...
  m_src_rect.SetRect(0, 0, 0, 0);
  m_dst_rect.SetRect(0, 0, 0, 0);
  m_video_stereo_mode = RenderStereoMode::OFF;
  m_display_stereo_mode = RenderStereoMode::OFF;
...
bool CMMALRenderer::RenderCapture(int index, CRenderCapture* capture)
{
  if (!m_bConfigured)
    return false;

  CLog::Log(LOGDEBUG, "{}::{} - {:p}", CLASSNAME, __func__, static_cast<void*>(capture));
...
bool CMMALRenderer::Supports(ERENDERFEATURE feature) const
{
...
bool CMMALRenderer::Supports(ESCALINGMETHOD method) const
{
  return false;
}
...
void CMMALRenderer::SetVideoRect(const CRect& InSrcRect, const CRect& InDestRect)
{
  ...
  RenderStereoMode video_stereo_mode = (m_iFlags & CONF_FLAGS_STEREO_MODE_SBS) ? RenderStereoMode::SPLIT_VERTICAL :
                                         (m_iFlags & CONF_FLAGS_STEREO_MODE_TAB) ? RenderStereoMode::SPLIT_HORIZONTAL : RenderStereoMode::OFF;
  bool stereo_invert                   = (m_iFlags & CONF_FLAGS_STEREO_CADANCE_RIGHT_LEFT) ? true : false;
  RenderStereoMode display_stereo_mode = CServiceBroker::GetWinSystem()->GetGfxContext().GetStereoMode();

  // ignore video stereo mode when 3D display mode is disabled
  if (display_stereo_mode == RenderStereoMode::OFF)
    video_stereo_mode = RenderStereoMode::OFF;
  ...
  CLog::Log(LOGDEBUG, "{}::{} {},{},{},{} -> {},{},{},{} (o:{} v:{} d:{} i:{})", CLASSNAME, __func__,
  ...
  if (display_stereo_mode == RenderStereoMode::SPLIT_VERTICAL)
  {
    float width = DestRect.x2 - DestRect.x1;
    DestRect.x1 *= 2.0f;
    DestRect.x2 = DestRect.x1 + 2.0f * width;
  }
  else if (display_stereo_mode == RenderStereoMode::SPLIT_HORIZONTAL)
  {
  ...
  if (m_video_stereo_mode == RenderStereoMode::SPLIT_HORIZONTAL)
    region.transform = (MMAL_DISPLAYTRANSFORM_T)(region.transform | DISPMANX_STEREOSCOPIC_TB);
  else if (m_video_stereo_mode == RenderStereoMode::SPLIT_VERTICAL)
  ...
  if (status != MMAL_SUCCESS)
    CLog::Log(LOGERROR, "{}::{} Failed to set display region (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));

  CLog::Log(LOGDEBUG, "{}::{} {},{},{},{} -> {},{},{},{} t:{:x}", CLASSNAME, __func__,
...
void CMMALRenderer::DestroyDeinterlace()
{
  ...
  if (m_deint_input && m_deint_input->is_enabled)
  {
    ...
    if (status != MMAL_SUCCESS)
      CLog::Log(LOGERROR, "{}::{} Failed to disable deinterlace input port(status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
  }
  ...
  if (m_deint_output && m_deint_output->is_enabled)
  {
    ...
    if (status != MMAL_SUCCESS)
      CLog::Log(LOGERROR, "{}::{} Failed to disable deinterlace output port(status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
  }
...
bool CMMALRenderer::CheckConfigurationDeint(uint32_t width, uint32_t height, uint32_t aligned_width, uint32_t aligned_height, uint32_t encoding, EINTERLACEMETHOD interlace_method, int bitsPerPixel)
{
  ...
  if (!m_bConfigured)
  {
    CLog::Log(LOGDEBUG, "{}::{} Unconfigured", CLASSNAME, __func__);
    return false;
  }

  if (!m_deint)
  {
    CLog::Log(LOGDEBUG, LOGVIDEO, "{}::{} CreateDeinterlace", CLASSNAME, __func__);
    ...
    {
      CLog::Log(LOGERROR, "{}::{} Failed to create pool for deint output", CLASSNAME, __func__);
      return false;
    }
    ...
    if (status != MMAL_SUCCESS)
      CLog::Log(LOGERROR, "{}::{} Failed to enable zero copy mode on {} (status={:x} {})", CLASSNAME, __func__, m_deint_input->name, status, mmal_status_to_string(status));
  }
  ...
  if (m_deint_input && (sizeChanged || deinterlaceChanged || encodingChanged))
  {
    ...
    CLog::Log(LOGDEBUG, "{}::{} Changing Deint dimensions from {}x{} ({}x{}) to {}x{} ({}x{}) {:4}->{:4} mode {}->{} bpp:{}", CLASSNAME, __func__,
        ...
        (char *)&m_deint_input->format->encoding, (char *)&encoding, (int)m_interlace_method, (int)interlace_method, bitsPerPixel);
    ...
    if (m_deint_input && m_deint_input->is_enabled)
    {
      ...
      if (status != MMAL_SUCCESS)
      {
        CLog::Log(LOGERROR, "{}::{} Failed to disable deint input port (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
        return false;
      }
    }
  }
  ...
  if (m_deint_output && (sizeChanged || deinterlaceChanged || encodingChanged))
  {
    if (m_deint_output && m_deint_output->is_enabled)
    {
      ...
      if (status != MMAL_SUCCESS)
      {
        CLog::Log(LOGERROR, "{}::{} Failed to disable deint output port (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
        return false;
      }
    }
  }
  ...
  if (m_deint_input && (sizeChanged || deinterlaceChanged || encodingChanged))
  {
    ...
    if (status != MMAL_SUCCESS)
    {
      CLog::Log(LOGERROR, "{}::{} Failed to commit deint input format (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
      return false;
    }
    ...
    if (!m_deint_input->is_enabled)
    {
      ...
      if (status != MMAL_SUCCESS)
      {
        CLog::Log(LOGERROR, "{}::{} Failed to enable deint input port (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
        return false;
      }
    }
  }
  ...
  if (m_deint_output && (sizeChanged || deinterlaceChanged || encodingChanged))
  {
    if (interlace_method != VS_INTERLACEMETHOD_NONE)
    {
      ...
      if (status != MMAL_SUCCESS)
      {
        CLog::Log(LOGERROR, "{}::{} Failed to set deinterlace parameters (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
        return false;
      }
      ...
      if (status != MMAL_SUCCESS)
        CLog::Log(LOGERROR, "{}::{} Failed to enable extra buffers on {} (status={:x} {})", CLASSNAME, __func__, m_deint_input->name, status, mmal_status_to_string(status));
    }
    else
    {
      ...
      if (status != MMAL_SUCCESS)
        CLog::Log(LOGERROR, "{}::{} Failed to configure MMAL_PARAMETER_CCM_SHIFT on {} (status={:x} {})", CLASSNAME, __func__, m_deint_input->name, status, mmal_status_to_string(status));
      status = mmal_port_parameter_set_uint32(m_deint_output, MMAL_PARAMETER_OUTPUT_SHIFT, 1);
      if (status != MMAL_SUCCESS)
        CLog::Log(LOGERROR, "{}::{} Failed to configure MMAL_PARAMETER_OUTPUT_SHIFT on {} (status={:x} {})", CLASSNAME, __func__, m_deint_output->name, status, mmal_status_to_string(status));
    }
  }
  ...
  if (m_deint_output && (sizeChanged || deinterlaceChanged || encodingChanged))
  {
    ...
    if (status != MMAL_SUCCESS)
    {
      CLog::Log(LOGERROR, "{}::{} Failed to commit deint output format (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
      return false;
    }
    ...
    if (!m_deint_output->is_enabled)
    {
      ...
      if (status != MMAL_SUCCESS)
      {
        CLog::Log(LOGERROR, "{}::{} Failed to enable deint output port (status={:x} {})", CLASSNAME, __func__, status, mmal_status_to_string(status));
        return false;
      }
    }
    if (m_deint_output_pool)
  ...
  if (m_deint && !m_deint->is_enabled)
  {
    ...
    if (status != MMAL_SUCCESS)
    {
      CLog::Log(LOGERROR, "{}::{} Failed to enable deinterlacer component {} (status={:x} {})", CLASSNAME, __func__, m_deint->name, status, mmal_status_to_string(status));
      return false;
    }
  }
